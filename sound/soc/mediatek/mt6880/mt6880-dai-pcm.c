...
static const struct snd_kcontrol_new mtk_pcm_2_playback_ch1_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("DL2_CH1", AFE_CONN17,
				    I_DL2_CH1, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ETDM_CAP_CH1", AFE_CONN17,
				    I_ETDM_CAP_CH1, 1, 0),
...
static const struct snd_kcontrol_new mtk_pcm_2_playback_ch2_mix[] = {
	...
	SOC_DAPM_SINGLE_AUTODISABLE("DL2_CH2", AFE_CONN18,
				    I_DL2_CH2, 1, 0),
	SOC_DAPM_SINGLE_AUTODISABLE("ETDM_CAP_CH1", AFE_CONN18,
				    I_ETDM_CAP_CH1, 1, 0),
			    ...
			    int event)
{
	...
	dev_err(afe->dev, "%s(), name %s, event 0x%x\n",
		 __func__, w->name, event);
	
	/*BEGIN add wakeup to keep power, by haowei.cheng*/
	if(event == SND_SOC_DAPM_PRE_PMU) {
		device_init_wakeup(afe->dev, true);
	} else {
		device_init_wakeup(afe->dev, false);
	}
	/*END add wakeup to keep power, by haowei.cheng*/
	return 0;
}
				 ...
				 struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6880_rate_transform(afe->dev, rate, dai->id);
