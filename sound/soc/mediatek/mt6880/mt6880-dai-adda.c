...
static int mtk_adda_ul_src_dmic(struct mtk_base_afe *afe, int id)
{
	...
	case MT6880_DAI_AP_DMIC:
		reg = AFE_ADDA_UL_SRC_CON0;
		break;
	case MT6880_DAI_AP_DMIC_CH34:
	default:
		return -EINVAL;
	}
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_ADDA_MTKAIF_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6880_DAI_ADDA);
		}
		break;
...
/* mtkaif dmic */
static const char * const mt6880_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6880_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6880_adda_off_on_str),
			    mt6880_adda_off_on_str),
};

static int mt6880_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6880_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6880_adda_enum[0],
		     mt6880_adda_dmic_get, mt6880_adda_dmic_set),
};
...
int mt6880_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6880_dai_set_priv(afe, MT6880_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT6880_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6880_DAI_ADDA];
