...
/*begin: modify by laizhenhao for PCM*/
				...
				int event)
{
	...
	struct mtk_afe_tdm_priv *tdm_priv = afe_priv->dai_priv[MT6880_DAI_TDM];
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6880_afe_gpio_request(afe, true, MT6880_DAI_TDM, 0);
		mt6880_mck_enable(afe, tdm_priv->bck_id, tdm_priv->bck_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6880_mck_disable(afe, tdm_priv->bck_id);
		mt6880_afe_gpio_request(afe, false, MT6880_DAI_TDM, 0);
		break;
	default:
		break;
	}
...
/*end: modify by laizhenhao for PCM*/
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6880_mck_enable(afe, tdm_priv->mclk_id, tdm_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		tdm_priv->mclk_rate = 0;
		mt6880_mck_disable(afe, tdm_priv->mclk_id);
		break;
	default:
		break;
	}
...
int mt6880_dai_tdm_register(struct mtk_base_afe *afe)
{
	...
	tdm_priv->bck_id = MT6880_TDM_BCK;
	tdm_priv->mclk_id = MT6880_TDM_MCK;

	afe_priv->dai_priv[MT6880_DAI_TDM] = tdm_priv;
	return 0;
}
