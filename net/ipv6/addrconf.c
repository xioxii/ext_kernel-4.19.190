...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len, bool sllao)
{
	...
	if (pinfo->onlink) {
		...
		if (rt) {
			...
			pr_info("accept_ra_prefix_route=%d\n", in6_dev->cnf.accept_ra_prefix_route);
			if (in6_dev->cnf.accept_ra_prefix_route) {
			    pr_info("addrconf_prefix_route called\n");
				...
				__s32 *array, int bytes)
{
	...
	array[DEVCONF_ACCEPT_RA_PREFIX_ROUTE] = cnf->accept_ra_prefix_route;
}
