...
extern void nf_ct_iterate_cleanup_net(struct net *net,
...
void trigger_timeout(struct timer_list *t)
{
...
static unsigned int trigger_out(struct sk_buff **pskb, const void *targinfo)
{
	...
	if (trig != NULL) {
        ...
        timer_setup(&trig->timeout,trigger_timeout,0);
...
static unsigned int trigger_dnat(struct sk_buff **pskb, unsigned int hooknum)
{
	...
	newrange.min_proto.all = trig->mproto;
	newrange.max_proto.all  = trig->mproto;
...
static struct xt_target redirect_reg = {
	...
	.hooks = (1 << NF_INET_LOCAL_IN) | (1 << NF_INET_FORWARD)|(1 << NF_INET_LOCAL_OUT) | (1 << NF_INET_PRE_ROUTING), 
