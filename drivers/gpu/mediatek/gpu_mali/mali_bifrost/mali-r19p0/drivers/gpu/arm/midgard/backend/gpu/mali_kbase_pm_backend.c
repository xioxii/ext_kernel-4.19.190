...
static void kbase_pm_gpu_poweroff_wait_wq(struct work_struct *data)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	dev_info(kbdev->dev, "[%s] poweroff_wait_in_progress "
			"set false \n", __func__);
...
void kbase_pm_do_poweroff(struct kbase_device *kbdev, bool is_suspend)
{
	...
	if (!kbdev->pm.backend.gpu_powered) {
		dev_info(kbdev->dev, "[%s] !gpu_powered and"
			"goto unlock_hwaccess\n", __func__);
		goto unlock_hwaccess;
	}

	if (kbdev->pm.backend.poweroff_wait_in_progress) {
		dev_info(kbdev->dev, "[%s] poweroff_wait_in_progress = true and"
			"goto unlock_hwaccess\n", __func__);
		goto unlock_hwaccess;
	}
	...
	dev_info(kbdev->dev, "[%s] poweroff_wait_in_progress set true\n", __func__);
...
static bool is_poweroff_in_progress(struct kbase_device *kbdev)
{
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	dev_info(kbdev->dev, "[%s] poweroff_wait_in_progress = %d\n",
		__func__, kbdev->pm.backend.poweroff_wait_in_progress);
