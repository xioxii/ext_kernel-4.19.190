...
static void drm_connector_get_cmdline_mode(struct drm_connector *connector)
{
	...
	if (fb_get_options(connector->name, &option)) {
		printk(KERN_DEBUG "drm_connector_get_cmdline_mode fb_get_options");
		return;
	}
	...
	if (!drm_mode_parse_command_line_for_connector(option,
						       connector,
						       mode)) {
		printk(KERN_DEBUG "drm_connector_get_cmdline_mode drm_mode_parse_command_line_for_connector failed");
		return;
	}
	...
	printk(KERN_DEBUG "cmdline mode for connector %s %dx%d@%dHz%s%s%s\n",
		      connector->name,
		      mode->xres, mode->yres,
		      mode->refresh_specified ? mode->refresh : 60,
		      mode->rb ? " reduced blanking" : "",
