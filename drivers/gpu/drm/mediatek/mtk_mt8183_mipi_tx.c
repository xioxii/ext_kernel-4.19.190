...
static int mtk_mipi_tx_pll_prepare(struct clk_hw *hw)
{
	...
	printk(KERN_DEBUG "mtk_mipi_tx_pll_prepare prepare: %u bps\n", mipi_tx->data_rate);
	...
	if (mipi_tx->data_rate >= 2000000000) {
		...
		printk(KERN_DEBUG "can't prepare and enable EINVAL\n");
		return -EINVAL;
	}

	ret = clk_prepare_enable(mipi_tx->ref_clk);
	if (ret < 0) {
		printk(KERN_DEBUG "can't prepare and enable mipi_tx ref_clk\n");
		dev_err(mipi_tx->dev,
			"can't prepare and enable mipi_tx ref_clk %d\n", ret);
		return ret;
	}

	printk(KERN_DEBUG "mtk_mipi_tx_pll_prepare 2\n");
	...
	printk(KERN_DEBUG "mtk_mipi_tx_pll_prepare done\n");
...
// MT6890
const struct mtk_mipitx_data mt6890_mipitx_data = {
	.mipi_tx_clk_ops = &mtk_mipi_tx_pll_ops,
	.mipi_tx_enable_signal = mtk_mipi_tx_power_on_signal,
	.mipi_tx_disable_signal = mtk_mipi_tx_power_off_signal,
};