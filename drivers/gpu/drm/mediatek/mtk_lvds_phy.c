...
static void mtk_lvds_tx_power_on_signal(struct phy *phy)
{
	...
	if (lvds_tx->dual_lvds || lvds_tx->lvds_on_tx1)
	...
	reg = RG_VPLL_EN | 1U << 8 | (lvds_tx->dual_lvds ? 0UL : 1UL) << 10 |
	      RG_VPLL_LVDS_EN | RG_VPLL_LVDS_DPIX_DIV2 |
	      (lvds_tx->dual_lvds ? 1UL : 0UL) << 16 | RG_VPLL_TXDIV5_EN |
...
static void mtk_lvds_tx_power_off_signal(struct phy *phy)
{
	...
	if (lvds_tx->dual_lvds || lvds_tx->lvds_on_tx1)
		writel(DA_LVDSTX_PWR_ON | LVDS_ISO_EN,
		       lvds_tx->tx1_regs + VOPLL_CTL3);

	writel(DA_LVDSTX_PWR_ON | LVDS_ISO_EN, lvds_tx->tx2_regs + VOPLL_CTL3);

	writel(0U, lvds_tx->tx2_regs + VOPLL_CTL2);
	if (lvds_tx->dual_lvds || lvds_tx->lvds_on_tx1)
		writel(0U, lvds_tx->tx1_regs + VOPLL_CTL2);
	...
	if (lvds_tx->dual_lvds || lvds_tx->lvds_on_tx1)
		writel(0U, lvds_tx->tx1_regs + LVDSTX_CTL3);
}
...
static int mtk_lvds_tx_probe(struct platform_device *pdev)
{
	...
	lvds_tx->lvds_on_tx1 = of_property_read_bool(dev->of_node,
						     "mediatek,lvds-on-low-channel");
