...
#define RDMA_MEM_GMC				0x401E001B //Original: 0xD2B4B478
...
static void mtk_rdma_layer_config(struct mtk_ddp_comp *comp,
	unsigned int idx, struct mtk_plane_pending_state *pending,
	void *handle)
{
	struct mtk_disp_rdma *rdma = comp_to_rdma(comp);
	unsigned int addr = (u32)pending->addr;
...
static const struct mtk_disp_rdma_data mt6890_rdma_driver_data = {
	.fifo_size = SZ_4K,
};
