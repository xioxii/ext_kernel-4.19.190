...
#define PA2_RG_CLKREF_REV_PLL_POSDIV	GENMASK(14, 12)
#define PA2_RG_CLKREF_REV_PLL_POSDIV_VAL(x)	((0x7 & (x)) << 12)
#define PA2_RG_CLKREF_REV_PLL_FBKSEL	BIT(11)
...
#define P2C_RG_USB20_DP_100K_MODE	BIT(18)
#define P2C_RG_USB20_DM_100K_EN		BIT(17)
#define P2C_USB20_DP_100K_EN		BIT(16)
...
#define U3P_U3_PHYA_REG11	0x044
#define RG_SSUSB_PLL_IBAND_FIX_EN	BIT(3)
...
#ifndef CONFIG_MTU3_MD_USB_EP0_CTRL
static int phy_efuse_settings(struct mtk_phy_instance *instance)
{
	struct u2phy_banks *u2_banks;
	struct u3phy_banks *u3_banks;
	...
	case PHY_TYPE_USB2:
		...
		efuse_field = "u2_efuse_data";
		break;

	case PHY_TYPE_USB3:
		u3_banks = &instance->u3_banks;
		efuse_field = "u3_efuse_data";
		break;
	default:
		pr_err("%s - NO efuse tuning for this PHY type!\n", __func__);
		return -1;
	}

	cell = nvmem_cell_get(dev, efuse_field);
	if (IS_ERR(cell)) {
		pr_err("%s - Cannot get efuse offset in dts\n", __func__);
	...
	nvmem_cell_put(cell);
	if (IS_ERR(buf)) {
		pr_err("%s - Cannot read efuse data!\n", __func__);
		return PTR_ERR(buf);
	}

	switch (instance->type) {
	case PHY_TYPE_USB2:
		pr_info("%s - u2_efuse_data: %x\n", __func__, *buf);
		evalue = (u32)(*buf) & 0x1f;
		if (evalue) {
		    tmp = readl(u2_banks->com + U3P_USBPHYACR1);
		    tmp &= ~PA1_RG_U2_INTR_CAL;
		    tmp |= PA1_RG_U2_INTR_CAL_VAL(evalue);
		    writel(tmp, u2_banks->com + U3P_USBPHYACR1);
		    pr_info("PA1_RG_U2_INTR_CAL=0x%x\n", tmp);
		}
		break;

	case PHY_TYPE_USB3:
		pr_info("%s - u3_efuse_data: %x\n", __func__, *buf);
		evalue = (u32)(*buf) & 0xf;
		if (evalue) {
		    tmp = readl(u3_banks->phyd + U3P_U3_PHYD_IMPCAL1);
		    tmp &= ~P3D_RG_SSUSB_RX_IMPSEL;
		    tmp |= P3D_RG_SSUSB_RX_IMPSEL_VAL(evalue);
		    writel(tmp, u3_banks->phyd + U3P_U3_PHYD_IMPCAL1);
		    pr_info("P3D_RG_SSUSB_RX_IMPSEL=0x%x\n", tmp);
		}
		evalue = ((u32)(*buf) & (0xf<<4)) >> 4;
		if (evalue) {
		    tmp = readl(u3_banks->phyd + U3P_U3_PHYD_IMPCAL0);
		    tmp &= ~P3D_RG_SSUSB_TX_IMPSEL;
		    tmp |= P3D_RG_SSUSB_TX_IMPSEL_VAL(evalue);
		    writel(tmp, u3_banks->phyd + U3P_U3_PHYD_IMPCAL0);
		    pr_info("P3D_RG_SSUSB_TX_IMPSEL=0x%x\n", tmp);
		}
		evalue = ((u32)(*buf) & (0x1f<<8)) >> 8;
		if (evalue) {
		    tmp = readl(u3_banks->phya + U3P_U3_PHYA_REG0);
		    tmp &= ~P3A_RG_SSUSB_IEXT_INTR_CTRL;
		    tmp |= P3A_RG_SSUSB_IEXT_INTR_CTRL_VAL(evalue);
		    writel(tmp, u3_banks->phya + U3P_U3_PHYA_REG0);
		    pr_info("P3A_RG_SSUSB_IEXT_INTR_CTRL=0x%x\n", tmp);
		}
		break;
	}
	kfree(buf);
...
static ssize_t rxsqth_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	struct mtk_phy_instance *instance = phy_get_drvdata(to_phy(dev));
	...
	dev_info(dev, "%s, rxsqth=%x\n", __func__, val);
	return count;
}

static ssize_t rxsqth_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct mtk_phy_instance *instance = phy_get_drvdata(to_phy(dev));
	struct u2phy_banks *u2_banks = &instance->u2_banks;
	...
	dev_info(dev, "%s, rxsqth=%s\n", __func__, str);
	return sprintf(buf, "rxsqth = %s\n", str);
}
static DEVICE_ATTR_RW(rxsqth);
	...
	struct mtk_phy_instance *instance)
{
	...
	tmp = readl(com + U3P_U2PHYACR4);
	tmp |= P2C_RG_USB20_DP_100K_MODE;
	writel(tmp, com + U3P_U2PHYACR4);

	tmp = readl(com + U3P_U2PHYACR4);
	tmp &= ~(P2C_USB20_DP_100K_EN | P2C_RG_USB20_DM_100K_EN);
	writel(tmp, com + U3P_U2PHYACR4);
	...
	tmp = readl(com + U3P_USBPHYACR6);
	tmp &= ~(PA6_RG_U2_PHY_REV6 | PA6_RG_U2_PHY_REV1);
	...
	struct mtk_phy_instance *instance)
{
	...
	tmp = readl(com + U3P_U2PHYDTM0);
	tmp &= ~(P2C_RG_DPPULLDOWN | P2C_RG_DMPULLDOWN | P2C_RG_XCVRSEL |
		P2C_RG_TERMSEL | P2C_RG_DATAIN |
		P2C_FORCE_DP_PULLDOWN | P2C_FORCE_DM_PULLDOWN |
		P2C_FORCE_XCVRSEL | P2C_FORCE_TERMSEL | P2C_FORCE_DATAIN);
	writel(tmp, com + U3P_U2PHYDTM0);
	...
	struct mtk_phy_instance *instance)
{
	...
	tmp |= (P2C_RG_DPPULLDOWN | P2C_RG_DMPULLDOWN |
		P2C_RG_XCVRSEL_VAL(1) | P2C_RG_TERMSEL |
		P2C_FORCE_DP_PULLDOWN | P2C_FORCE_DM_PULLDOWN |
		P2C_FORCE_XCVRSEL | P2C_FORCE_TERMSEL | P2C_FORCE_DATAIN);
	writel(tmp, com + U3P_U2PHYDTM0);

	tmp = readl(com + U3P_USBPHYACR6);
	tmp |= (PA6_RG_U2_PHY_REV6_VAL(1) | PA6_RG_U2_PHY_REV1);
	writel(tmp, com + U3P_USBPHYACR6);
				...
				struct mtk_phy_instance *instance)
{
	...
	device_property_read_u32(dev, "mediatek,rx-sqth",
				 &instance->rx_sqth);
	dev_info(dev, "bc12:%d src:%d vrt:%d term:%d rev6:%d disc:%d sqth:%d\n",
		instance->bc12_en, instance->eye_src,
		instance->eye_vrt, instance->eye_term,
		instance->eye_rev6, instance->eye_disc, instance->rx_sqth);
}
			     ...
			     struct mtk_phy_instance *instance)
{
	...
	tmp &= ~PA2_RG_CLKREF_REV_PLL_POSDIV;
	tmp |= PA2_RG_CLKREF_REV_PLL_POSDIV_VAL(1);
	tmp |= PA2_RG_CLKREF_REV_PLL_FBKSEL;
