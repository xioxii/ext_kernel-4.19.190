...
static int mtk_smi_clk_enable(const struct mtk_smi *smi)
{
	...
	pr_info("[SMI LOG] Start SMI CLK_EN\n");
	ret = clk_prepare_enable(smi->clk_apb);
	if (ret){
		pr_err("Failed to get smi->clk_apb\n");
	}
	pr_info("[SMI LOG] Start SMI CLK_EN-smi->clk_smi\n");
	ret = clk_prepare_enable(smi->clk_smi);
	if (ret){
		pr_err("Failed to get smi->clk_smi\n");
	}
	pr_info("[SMI LOG] Start SMI CLK_EN-smi->clk_gals0\n");
	ret = clk_prepare_enable(smi->clk_gals0);
	if (ret){
		pr_err("Failed to get smi->clk_gals0\n");
	}
	pr_info("[SMI LOG] Start SMI CLK_EN-smi->clk_gals1\n");
	ret = clk_prepare_enable(smi->clk_gals1);
	if (ret){
		pr_err("Failed to get smi->smi->clk_gals1\n");
	}*/
...
static int mtk_smi_larb_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(larb->base))
		pr_err("Failed to get larb->base\n");

	larb->smi.clk_apb = devm_clk_get(dev, "apb");
	if (IS_ERR(larb->smi.clk_apb))
		pr_err("Failed to get larb->smi.clk_apb\n");

	larb->smi.clk_smi = devm_clk_get(dev, "smi");
	if (IS_ERR(larb->smi.clk_smi))
		pr_err("Failed to get larb->smi.clk_smi\n");
	...
	if (larb->larb_gen->has_gals) {
		...
		if (PTR_ERR(larb->smi.clk_gals0) == -ENOENT)
			...
			pr_err("Failed to get larb->smi.clk_gals0\n");
	}
	larb->smi.dev = dev;
	...
	pr_info("[SMI LOG] smi larb probe done\n");
...
static int mtk_smi_common_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(common->clk_apb))
		pr_err("Failed to get common->clk_apb\n");

	common->clk_smi = devm_clk_get(dev, "smi");
	if (IS_ERR(common->clk_smi))
		pr_err("Failed to get common->clk_smi\n");

	if (common->plat->has_gals) {
		common->clk_gals0 = devm_clk_get(dev, "gals0");
		if (IS_ERR(common->clk_gals0))
			pr_err("Failed to get common->clk_gals0\n");

		common->clk_gals1 = devm_clk_get(dev, "gals1");
		if (IS_ERR(common->clk_gals1))
			pr_err("Failed to get common->clk_gals1\n");
	}
	...
	if (common->plat->gen == MTK_SMI_GEN1) {
		...
		if (IS_ERR(common->smi_ao_base))
			pr_err("Failed to get common->smi_ao_base\n");

		common->clk_async = devm_clk_get(dev, "async");
		if (IS_ERR(common->clk_async))
			pr_err("Failed to get common->clk_async\n");
		...
		if (IS_ERR(common->base))
			pr_err("Failed to get common->base\n");
	}
	pm_runtime_enable(dev);
...
static int __maybe_unused mtk_smi_common_resume(struct device *dev)
{
	...
	wmb();*/ /* make sure settings are written */
	pr_info("[SMI LOG] mtk_smi_common_resume Done\n");
	return 0;
}
...
{
	int ret;
	pr_info("[SMI LOG] Start to register-COMMON SMI driver\n");
	ret = platform_driver_register(&mtk_smi_common_driver);
	if (ret != 0) {
		pr_err("Failed to register SMI driver\n");
		return ret;
	}
	pr_info("[SMI LOG] Start to register SMI-LARB driver\n");
	ret = platform_driver_register(&mtk_smi_larb_driver);
	if (ret != 0) {
		pr_err("Failed to register SMI-LARB driver\n");
		goto err_unreg_smi;
	}
	pr_info("[SMI LOG] smi init done\n");
	return ret;
