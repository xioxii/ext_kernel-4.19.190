...
#define CMDQ_GET_ADDR_H(addr)		(sizeof(addr) > sizeof(uint32_t) ? (addr >> 32) : 0)
...
int cmdq_pkt_timer_en(struct cmdq_pkt *pkt)
{
	...
	return cmdq_pkt_write_ex(pkt, NULL, gce_pa + CMDQ_TPR_MASK, en, en);
}
	...
	u16 tick, u16 reg_gpr)
{
	...
	cmdq_pkt_logic_command(pkt, CMDQ_LOGIC_OR, CMDQ_CPR_TPR_MASK,
		&lop, &rop);
	cmdq_pkt_write_indriect(pkt, clt_base, timeout_en, CMDQ_CPR_TPR_MASK, ~0);
	...
	u32 value, u32 addr, u32 mask, u16 count, u16 reg_gpr)
{
	...
	cmdq_pkt_sleep(pkt, clt_base, CMDQ_POLL_TICK, reg_gpr);
