...
static int uether_debug = 0;
module_param(uether_debug, uint, 0644);
MODULE_PARM_DESC (uether_debug, "enable uether debugging");

#define U_ETHER_DBG(fmt, args...) \
		if (uether_debug) \
			pr_emerg("U_ETHER,%s, " fmt, __func__, ## args)

static int ip_debug = 0;
module_param(ip_debug, uint, 0644);
MODULE_PARM_DESC (ip_debug, "enable ip_debug debugging");

static int ip_debug_ratelimit = 0;
module_param(ip_debug_ratelimit, uint, 0644);
MODULE_PARM_DESC (ip_debug_ratelimit, "enable ip_debug_ratelimit debugging");

static int uether_tx_profile = 0;
module_param(uether_tx_profile, uint, 0644);
MODULE_PARM_DESC (uether_tx_profile, "enable uether_tx_profile debugging");

static int uether_tx_debug = 0;
module_param(uether_tx_debug, uint, 0644);
MODULE_PARM_DESC (uether_tx_debug, "enable uether_tx_debug debugging");

static int uether_tx_busy_profile = 0;
module_param(uether_tx_busy_profile, uint, 0644);
MODULE_PARM_DESC (uether_tx_busy_profile, "enable uether_tx_busy_profile debugging");

static int uether_tx_busy_profile_ratelimit = 0;
module_param(uether_tx_busy_profile_ratelimit, uint, 0644);
MODULE_PARM_DESC (uether_tx_busy_profile_ratelimit, "enable uether_tx_busy_profile_ratelimit debugging");

static int uether_tx_req_alloc_debug = 0;
module_param(uether_tx_req_alloc_debug, uint, 0644);
MODULE_PARM_DESC (uether_tx_req_alloc_debug, "enable uether_tx_req_alloc_debug debugging");


static int uether_rx_debug = 0;
module_param(uether_rx_debug, uint, 0644);
MODULE_PARM_DESC (uether_rx_debug, "enable uether_rx_debug debugging");

static int uether_rx_profile;
module_param(uether_rx_profile, uint, 0644);
MODULE_PARM_DESC(uether_rx_profile, "enable uether_rx_profile debugging");

static int uether_rx_submit_debug = 0;
module_param(uether_rx_submit_debug, uint, 0644);
MODULE_PARM_DESC(uether_rx_submit_debug, "enable uether_rx_submit_debug debugging");

static int uether_rx_complete_profile = 0;
module_param(uether_rx_complete_profile, uint, 0644);
MODULE_PARM_DESC (uether_rx_complete_profile, "enable uether_rx_complete_profile debugging");

static int rndis_mtu = 0;
module_param(rndis_mtu, uint, 0644);
MODULE_PARM_DESC (rndis_mtu, "enable rndis_mtu debugging");

static int rndis_gso = 1;
module_param(rndis_gso, uint, 0644);
MODULE_PARM_DESC (rndis_gso, "enable rndis_gso debugging");

static int rndis_gso_debug = 0;
module_param(rndis_gso_debug, uint, 0644);
MODULE_PARM_DESC (rndis_gso_debug, "enable rndis_gso_debug debugging");

static int uether_rx_error_debug = 0;
module_param(uether_rx_error_debug, uint, 0644);
MODULE_PARM_DESC (uether_rx_error_debug, "enable uether_rx_error_debug debugging");

static int uether_tx_rtt_profile = 0;
module_param(uether_tx_rtt_profile, uint, 0644);
MODULE_PARM_DESC (uether_tx_rtt_profile, "enable uether_tx_rtt_profile debugging");

static int uether_rx_rtt_profile = 0;
module_param(uether_rx_rtt_profile, uint, 0644);
MODULE_PARM_DESC (uether_rx_rtt_profile, "enable uether_rx_rtt_profile debugging");

static int uether_rx_frames_qlen_threshold = 1500;
module_param(uether_rx_frames_qlen_threshold, uint, 0644);
MODULE_PARM_DESC (uether_rx_frames_qlen_threshold, "define uether_rx_frames_qlen_threshold value");

static int uether_rx_work_rtt_profile = 0;
module_param(uether_rx_work_rtt_profile, uint, 0644);
MODULE_PARM_DESC (uether_rx_work_rtt_profile, "enable uether_rx_work_rtt_profile debugging");

static int u_ether_tx_req_threshold = 1;
module_param(u_ether_tx_req_threshold, uint, 0644);
MODULE_PARM_DESC (u_ether_tx_req_threshold, "enable u_ether_tx_req_threshold debugging");
...
rx_submit(struct eth_dev *dev, struct usb_request *req, gfp_t gfp_flags)
{
	...
	if (uether_rx_submit_debug) {
		#if 0
			pr_emerg("RNDIS RX submit - ethhdr=%lu, mtu=%d, header_len=%d, maxpacket-1=%d, ul_max_pkts_per_xfer=%d,  is_fixed=%d, fixed_out_len=%d,  size=%lu\n",
					sizeof(struct ethhdr), dev->net->mtu, dev->port_usb->header_len, out->maxpacket-1, dev->ul_max_pkts_per_xfer, dev->port_usb->is_fixed,
					dev->port_usb->fixed_out_len, size);
		#else
			pr_emerg("RNDIS RX submit - rx.frames.qlen=%u,  size=%lu\n", dev->rx_frames.qlen, size);
		#endif
	}
...
static void rx_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	ktime_t rtt_rx_start_time, rtt_rx_queue_time, rtt_rx_delta_time;
	unsigned long long rtt_rx_duration;

	if (uether_rx_debug) {
		pr_emerg("RX[%lu,%lu,%lu,%lu] TX[%lu,%lu,%lu,%lu,%lu,%lu]\n",
		...
		rndis_test_tx_busy, rndis_test_tx_nomem, rndis_test_tx_stop,
		rndis_test_tx_complete);
	}

	if (uether_rx_rtt_profile)	{
		rtt_rx_start_time = ktime_get();
	}
		...
		if (dev->unwrap) {
			...
			if (dev->port_usb) {
				...
				if (status == -EINVAL) {
					...
					if (uether_rx_error_debug)
						pr_emerg("%s - after rm_hdr - rx_error:%lu\n", __func__, dev->net->stats.rx_errors);
				}
...
quiesce:
	...
	default:
		...
		dev->net->stats.rx_errors++;
		if (uether_rx_error_debug)
			pr_emerg("%s - default - rx_error:%lu\n", __func__, dev->net->stats.rx_errors);
		DBG(dev, "rx status %d\n", status);
		break;
	}
...
clean:
	...
	if (queue) {
		if (uether_rx_rtt_profile)	{
			rtt_rx_queue_time = ktime_get();
			rtt_rx_delta_time = ktime_sub(rtt_rx_queue_time, rtt_rx_start_time);
			rtt_rx_duration = (unsigned long long) ktime_to_ns(rtt_rx_delta_time) >> 10;
			pr_emerg("RNDIS rtt_rx_duration=%llu (queue_work)\n", rtt_rx_duration);
		}
		queue_work(uether_wq, &dev->rx_work);
		if (dev->rx_frames.qlen < uether_rx_frames_qlen_threshold) {
...
static int alloc_requests(struct eth_dev *dev, struct gether *link, unsigned n)
{
	...
	if (status < 0) {
		...
		pr_emerg("can't alloc tx requests\n");
		return status;
	}
	...
	if (status < 0) {
		spin_unlock(&dev->reqrx_lock);
		pr_emerg("can't alloc rx requests\n");
		return status;
	}
	spin_unlock(&dev->reqrx_lock);
	return status;
...
int eth_hw_nat(bool to_usb, struct sk_buff *skb, struct net_device *net)
{
	...
	if (to_usb) {
		if (ppe_hook_tx_rndis)
			return ppe_hook_tx_rndis(skb);
		else
			return 1;
	} else {
		if (ppe_hook_rx_rndis && IS_SPACE_AVAILABLE_HEAD(skb)) {
			FOE_MAGIC_TAG(skb) = FOE_MAGIC_RNDIS;

			return ppe_hook_rx_rndis(skb);
		} else
			return 1;
	}
}

static void print_rx_profile_log(struct eth_dev	*dev,
		struct sk_buff *skb)
{
	long long time_diff;

	if (dev->rx_frames.qlen > dev->rx_profile_max_rx_pending_num)
		dev->rx_profile_max_rx_pending_num = dev->rx_frames.qlen;
	dev->rx_profile_tput += skb->len;
	dev->rx_profile_packet_num++;

	time_diff = sched_clock() - dev->rx_profile_ts;
	if (time_diff > 1000000000L) {
		pr_emerg("U_ETHER RX t=%lld pend=%d num=%lld %lldMbps\n",
			time_diff, dev->rx_profile_max_rx_pending_num,
			dev->rx_profile_packet_num,
			(dev->rx_profile_tput * 8) / (time_diff/1000));
		dev->rx_profile_ts += time_diff;
		dev->rx_profile_tput = 0;
		dev->rx_profile_packet_num = 0;
		dev->rx_profile_max_aggregation_num = 1;
		dev->rx_profile_max_rx_pending_num = 0;
	}
}
...
static void process_rx_w(struct work_struct *work)
{
	...
	if (uether_rx_work_rtt_profile)	{
		rtt_rx_start_time = ktime_get();
	}

	while ((skb = skb_dequeue(&dev->rx_frames))) {
		if (uether_rx_profile)
			print_rx_profile_log(dev, skb);

		if (dev->rx_frames.qlen < (uether_rx_frames_qlen_threshold/2)) {
			U_ETHER_DBG("%s - after skb_dequeue - dev->rx_frames.qlen:%d < uether_rx_frames_qlen_threshold/2", __func__, dev->rx_frames.qlen);
			queue_work(uether_wq1, &dev->rx_work1);
		}
		if (uether_rx_error_debug)
			pr_emerg("%s - after skb_dequeue - rx_error:%lu, status:%d, skb->len:%d\n", __func__, dev->net->stats.rx_errors, status, skb->len);
		if (status < 0
				|| ETH_HLEN > skb->len
				|| skb->len > ETH_FRAME_LEN) {
			dev->net->stats.rx_errors++;
			if (uether_rx_error_debug)
				pr_emerg("%s - after skb_dequeue error - rx_error:%lu, status:%d, skb->len:%d\n", __func__, dev->net->stats.rx_errors, status, skb->len);
...
static int alloc_tx_buffer(struct eth_dev *dev)
{
	...
	return 0;
free_buf:
	if (uether_tx_req_alloc_debug) {
		pr_emerg("RNDIS alloc_tx_buffer fail\n");
	}
	rndis_test_tx_nomem++;
...
static void print_tx_profile_log(struct eth_dev *dev,
		struct sk_buff *skb, struct net_device *net)
{
	...
	if (qdisc->q.qlen > dev->tx_profile_max_net_queue_num)
		dev->tx_profile_max_net_queue_num = qdisc->q.qlen;
	dev->tx_profile_tput += skb->len;
	dev->tx_profile_packet_num++;

	time_diff = sched_clock() - dev->tx_profile_ts;
	if (time_diff > 1000000000L) {
		pr_emerg("U_ETHER TX t=%lld pend=%d num=%lld %lldMbps\n",
			time_diff, dev->tx_profile_max_net_queue_num,
			dev->tx_profile_packet_num,
			(dev->tx_profile_tput*8)/(time_diff/1000));
		dev->tx_profile_ts += time_diff;
		dev->tx_profile_tput = 0;
		dev->tx_profile_packet_num = 0;
		dev->tx_profile_max_net_queue_num = 0;
	}
}
					...
					struct net_device *net)
{
	...
	static DEFINE_RATELIMIT_STATE(ratelimit4, 1 * HZ, 2);
	struct iphdr *iph;
	...
	if (ip_debug)
		if ((ip_debug_ratelimit && __ratelimit(&ratelimit4)) ||
			!ip_debug_ratelimit)
			pr_emerg("len=0x%x, id=0x%x, frag= 0x%x\n",
				iph->tot_len, iph->id, iph->frag_off);

	if (uether_tx_profile)
		print_tx_profile_log(dev, skb, net);
	...
	if (list_empty(&dev->tx_reqs)) {
		busyCnt++;
		if (uether_tx_busy_profile)
			if (!uether_tx_busy_profile_ratelimit ||
				(uether_tx_busy_profile_ratelimit &&
				__ratelimit(&ratelimit2)))
				pr_emerg("okCnt : %lu, busyCnt : %lu,"
					"tx_busy : %lu\n",
	...
	if (multi_pkt_xfer) {
		...
		if (net->features & NETIF_F_GSO) {
			...
			if (rndis_gso_debug) {
				pr_emerg("RNDIS Enable NETIF_F_GSO, frag_cnt: %d\n", frag_cnt);
			}
		}
		if (frag_cnt == 0) {
			...
			if (rndis_gso_debug) {
				pr_emerg("FRAG[-] req->length: %d, skb->len: %d, skb->data_len: %d\n", req->length, skb->len, skb->data_len);
			}
			for (frag_idx = 0; frag_idx < frag_cnt; frag_idx++) {
				...
				if (rndis_gso_debug) {
					pr_emerg("FRAG[%d] frag_data_len: %d, frag_data_addr: %p\n", frag_idx, frag_data_len, frag_data_addr);
				}
				...
				if (rndis_gso_debug) {
					pr_emerg("FRAG[%d] frag_total_len: %d, req->length: %d, Last frag_data_addr: %p\n", frag_idx, frag_total_len, req->length, frag_data_addr);
				}
			}

			if (rndis_gso_debug) {
				pr_emerg("FRAG: req->length: %d\n", req->length);
			}
		}
		...
		if ((dev->tx_skb_hold_count < dev->dl_max_pkts_per_xfer)
			...
			if (dev->no_tx_req_used > u_ether_tx_req_threshold) {
				...
				spin_unlock_irqrestore(&dev->req_lock, flags);
				if (uether_tx_rtt_profile)	{
					rtt_tx_queue_time = ktime_get();
					rtt_tx_delta_time = ktime_sub(rtt_tx_queue_time, rtt_tx_start_time);
					rtt_tx_duration = (unsigned long long) ktime_to_ns(rtt_tx_delta_time) >> 10;
					pr_emerg("RNDIS rtt_tx_duration=%llu (aggregation)\n", rtt_tx_duration);
				}
				goto success;
			}
		}
	...
	if (uether_tx_rtt_profile)	{
			...
			rtt_tx_delta_time = ktime_sub(rtt_tx_queue_time, rtt_tx_start_time);
			rtt_tx_duration = (unsigned long long) ktime_to_ns(rtt_tx_delta_time) >> 10;
			pr_emerg("RNDIS rtt_tx_duration=%llu\n", rtt_tx_duration);
	}
...
struct net_device *gether_setup_name_default(const char *netname)
{
	...
	pr_emerg("%s - dev->qmult:%d, qmult:%d\n", __func__, dev->qmult, qmult);
	snprintf(net->name, sizeof(net->name), "%s%%d", netname);
