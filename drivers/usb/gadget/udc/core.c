...
{
	#ifdef CONFIG_MTU3_MD_USB_EP0_CTRL
	struct mtu3 *mtu = container_of(gadget, struct mtu3, g);
	struct mtu3_md_sync_data *md_sync_data = mtu->md_sync_data;
	int tx_ep = md_sync_data->tx_ep;
	int rx_ep = md_sync_data->rx_ep;
	u8 epnum = simple_strtoul(&ep->name[2], NULL, 10);
	#endif
	u8		type;
	u16		max;
	...
	#ifdef CONFIG_MTU3_MD_USB_EP0_CTRL
	dev_dbg(gadget->dev.parent, "ep_name:%s, epnum:%d, type:0x%X, max:0x%X\n", ep->name, epnum, type, max);
	dev_dbg(gadget->dev.parent, "tx_ep:0x%X, rx_ep:0x%X\n", tx_ep, rx_ep);
	if (ep->caps.dir_in && ((1 << epnum) & tx_ep))
			dev_dbg(gadget->dev.parent, "match IN port:%d\n", epnum);
	else if (ep->caps.dir_out && ((1 << epnum) & rx_ep))
			dev_dbg(gadget->dev.parent, "match OUT port:%d\n", epnum);
	else {
		dev_dbg(gadget->dev.parent, "NO match port:%d\n", epnum);
		return 0;
	}
	#endif
