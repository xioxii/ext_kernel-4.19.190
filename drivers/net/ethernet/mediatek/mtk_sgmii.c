...
int mtk_sgmii_setup_mode_force(struct mtk_sgmii *ss, int id, struct mtk_eth *eth)
{
	unsigned int val;
	int mode;
	...
	/* SGMII force mode setting  Gen1 need adjust sgmii speed*/
	if((NULL != ss->sgmii_link_adjust)&&(mode == MTK_SGMII_PHYSPEED_1000)){
		ss->sgmii_link_adjust(ss,id);
	}else{
		val = 0x31120009;
		pr_info("set sgmii phy mode=0x%08x\n",val);
		regmap_write(ss->regmap[id], SGMSYS_SGMII_MODE, val);
	}
