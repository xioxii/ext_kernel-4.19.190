			...
			const struct pci_device_id *pci_id)
{
	...
	if (dev_get_by_name(&init_net, AQR_DEV_NAME) == NULL) {
		dev_info(&pdev->dev, "[HS ethernet/AQR] dev name %s ...!!\n", AQR_DEV_NAME);
		strncpy(self->ndev->name, AQR_DEV_NAME, sizeof(self->ndev->name) - 1);
	} else {
		dev_info(&pdev->dev, "[HS ethernet/AQR] dev name %s ...!!\n", AQR_DEV2_NAME);
		strncpy(self->ndev->name, AQR_DEV2_NAME, sizeof(self->ndev->name) - 1);
	}
