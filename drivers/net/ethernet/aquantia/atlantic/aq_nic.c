...
int aq_nic_xmit(struct aq_nic_s *self, struct sk_buff *skb)
{
	...
	if (strcmp(aq_nic_get_ndev(self)->name, AQR_DEV_NAME) == 0) {
		port_num = 11;
	} else if (strcmp(aq_nic_get_ndev(self)->name, AQR_DEV2_NAME) == 0) {
		port_num = 12;
	} else {
		pr_info("port_num error");
	}

//#if defined(CONFIG_RA_HW_NAT_AQR_SUPPORT)
	if (ppe_hook_tx_ext != NULL) {
		if (ppe_hook_tx_ext(skb, port_num) != 1) {
