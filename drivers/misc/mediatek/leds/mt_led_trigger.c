	...
	struct device_attribute *attr, char *buf)
{
	...
	case CC_MODE_ATTR_SFSTR:
		ret = info->ops->get_current_step(info);
		break;
	default:
		return -EINVAL;
	}
...
static int mt_led_cc_activate(struct led_classdev *led)
{
	...
	if ((info->magic_code & MT_LED_MAGIC_MASK) == MT_LED_MAGIC_CODE &&
		...
		//check ops
		if(NULL == info->ops){
			dev_err(led->dev, "%s: operation is NULL\n", __func__);
			return 0;
		}
	...
	struct device_attribute *attr, char *buf)
{
	...
	case PWM_MODE_ATTR_DIM_FREQ:
		...
		value_HZ = (1000*precision)/((ret+1)*2);
...
MODULE_AUTHOR("SR Huang <sr.huang@mediatek.com>");
MODULE_DESCRIPTION("MT6330 PMIC LED Trigger");
MODULE_LICENSE("GPL");
