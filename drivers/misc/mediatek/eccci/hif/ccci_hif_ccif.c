...
{
	unsigned reg_value;
	unsigned int reset_bit = 8;
	int i;

	/*set reset bit*/
	reg_value = ccif_read32((infra_ao_base + INFRA_RST0_REG_PD),
		CCIF_HW_RESET_SET);
	reg_value |= (1 << reset_bit);
	ccif_write32((infra_ao_base + INFRA_RST0_REG_PD),
		CCIF_HW_RESET_SET ,reg_value);
	/*clear reset bit*/
	reg_value = ccif_read32((infra_ao_base + INFRA_RST0_REG_PD),
		CCIF_HW_RESET_CLR);
	reg_value |= (1 << reset_bit);
	ccif_write32((infra_ao_base + INFRA_RST0_REG_PD),
		CCIF_HW_RESET_CLR, reg_value);
	/* clear SRAM */
	for (i = 0; i < CCIF_SRAM_SIZE/sizeof(unsigned int); i++) {
		ccif_write32(baseA, APCCIF_CHDATA+i*sizeof(unsigned int), 0);
		ccif_write32(baseB, APCCIF_CHDATA+i*sizeof(unsigned int), 0);
	}
}
...
void md_ccif_reset_queue(unsigned char hif_id, unsigned char for_start)
{
	...
	if (for_start) {
		...
		// no need to clear SRAM data, otherwise there was no data for exception dump
		// ccif hw will be reset at the beginning
	...
	struct sk_buff *skb, int skb_from_pool, int blocking)
{
	...
	if (NULL == skb){
		md_ccif_send(hif_id, qno);
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG,
			"trigger ccif interrupt %d\n", qno);
		return 0;
	}
	if (qno == 0xFF)
		return -CCCI_ERR_INVALID_QUEUE_INDEX;
...
int ccci_ccif_exception(unsigned char hif_id,
	enum HIF_EX_STAGE stage)
{
	switch (stage) {
	case HIF_EX_INIT:
		md_ccif_op_dump_status(hif_id,
...
int ccci_ccif_ex_handshake(unsigned char md_id, unsigned char hif_id,
	enum HIF_EX_STAGE stage)
{
	switch (stage) {
	case HIF_EX_INIT:
		md_ccif_send(hif_id, H2D_EXCEPTION_ACK);
		CCCI_DEBUG_LOG(md_id, TAG, "send ex ack to MD\n");
		break;
	case HIF_EX_INIT_DONE:
		break;
	case HIF_EX_CLEARQ_DONE:
		md_ccif_send(hif_id, H2D_EXCEPTION_CLEARQ_ACK);
		CCCI_DEBUG_LOG(md_id, TAG, "send ex clearq_ack to MD\n");
		break;
	case HIF_EX_ALLQ_RESET:
		break;
	default:
...
int ccci_ccif_hif_late_init( unsigned char hif_id)
{
	md_ccif_ring_buf_init(hif_id);
...
void ccci_ccif_reset(unsigned char hif_id, char is_hw)
{
	struct md_ccif_ctrl *md_ctrl =
		(struct md_ccif_ctrl *)ccci_hif_get_by_id(hif_id);
	struct ccci_smem_region *mdss_dbg;
	if(1 == is_hw){
		ccci_reset_ccif_hw(md_ctrl->md_id, hif_id,
			md_ctrl->ccif_ap_base,
			md_ctrl->ccif_md_base);
         /* 0~60 bytes for bootup trace
                 * last 12bytes for magic pattern, smem address and size.
		 */
		 CCCI_NOTICE_LOG(-1, TAG, "CCIF RESET HW\n");
		 mdss_dbg = ccci_md_get_smem_by_user_id(
		 	md_ctrl->md_id, SMEM_USER_RAW_MDSS_DBG);
		 ccif_write32((char *)(md_ctrl->ccif_sram_layout),
		 	(md_ctrl->sram_size - 3 * sizeof(u32)), 0x7274626E);
		 ccif_write32((char *)(md_ctrl->ccif_sram_layout),
		 	(md_ctrl->sram_size - 2 * sizeof(u32)),
		 	mdss_dbg->base_md_view_phy);
		 ccif_write32((char *)(md_ctrl->ccif_sram_layout),
		 	(md_ctrl->sram_size - sizeof(u32)), mdss_dbg->size);
	}else{		
		md_ccif_switch_ringbuf(hif_id, RB_EXP);
		md_ccif_reset_queue(hif_id, 1);
		md_ccif_switch_ringbuf(hif_id, RB_NORMAL);
		md_ccif_reset_queue(hif_id, 1);
    }
}
