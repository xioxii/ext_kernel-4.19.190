	...
	struct dpmaifq_normal_pit *pkt_inf_t)
{
	...
	struct dpmaif_bat_skb_t *cur_skb_info = ((struct dpmaif_bat_skb_t *)
		rxq->bat_req.bat_skb_ptr + (pkt_inf_t->buffer_id|((pkt_inf_t->h_bid)<<13) ));
#else
	struct dpmaif_bat_skb_t *cur_skb_info = ((struct dpmaif_bat_skb_t *)
...
static int dpmaif_net_rx_push_thread(void *arg)
{
	...
	while (1) {
		if (skb_queue_empty(&queue->skb_list.skb_list)) {
			...
			/*before wait_event_interruptible, relax wake lock*/
...
fast_retry:
	...
	CCCI_HISTORY_LOG(-1, TAG, "%s idx: 0x%x -> 0x%x, 0x%x, 0x%x\n",
		__func__, bat_req->pkt_buf_sz, new_skb->len,
...
#if defined(CONFIG_HW_NAT) && defined(_DPMAIF_MED_SUPPORT_)
	if (ppe_hook_rx_modem) {
		ppe_hook_rx_modem(new_skb, 0, rxq->cur_chn_idx);
	}
#endif
...
#if defined(_DPMAIF_MED_SUPPORT_) && defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
static void dpmaif_ipi_rx_request(int id, void *data, unsigned int len)
{
...
void ccci_dpmaif_notify(int flag)
{
...
retry:
	...
	if (ppe_hook_tx_modem) {
		ppe_hook_tx_modem(skb, 0, ccci_h.data[0]); //to-do
	}
#endif
...
static int dpmaif_resume(unsigned char hif_id)
{
	...
	if (drv_dpmaif_check_power_down() == false) {
		...
		CCCI_NORMAL_LOG(0, TAG, "sys_resume power down before\n");
#if !defined(_DPMAIF_GEN98_CODA_)
		/*flush and release UL descriptor*/
		for (i = 0; i < DPMAIF_TXQ_NUM; i++) {
