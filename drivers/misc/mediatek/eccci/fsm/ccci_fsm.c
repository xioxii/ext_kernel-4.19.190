	...
	struct ccci_fsm_command *cmd)
{
	...
	ctl->boot_count++;
	count = 0;
	hs_timeout = BOOT_TIMEOUT;
	while (count < hs_timeout/EVENT_POLL_INTEVAL) {
		spin_lock_irqsave(&ctl->event_lock, flags);
		if (!list_empty(&ctl->event_queue)) {
			event = list_first_entry(&ctl->event_queue,
						struct ccci_fsm_event, entry);
	...
	struct ccci_fsm_command *cmd)
{
	...
	if (is_epon_set) {
		...
		panic("WDT trigger KE");
		CCCI_NORMAL_LOG(ctl->md_id, FSM,
				"trigger exception dump after WDT\n");
