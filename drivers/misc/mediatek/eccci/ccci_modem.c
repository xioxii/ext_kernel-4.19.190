...
{SMEM_USER_CCISM_MCU,		(4*1024+120)*1024, (720+1)*1024,	0, },
{SMEM_USER_CCISM_MCU_EXP,	(4*1024+841)*1024, (120+1)*1024,	0, },
...
static void ccci_6297_md_smem_layout_config(struct ccci_modem *md)
{
	...
	(sizeof(md1_6297_noncacheable_fat)/sizeof(struct ccci_smem_region));
		i++) {
		if (md1_6297_noncacheable_fat[i].id == SMEM_USER_RAW_PHY_CAP) {
	...
	struct md_query_ap_feature *md_feature)
{
	...
	md_feature->feature_set[MD_MTEE_SHARE_MEMORY_ENABLE].support_mask
		= CCCI_FEATURE_OPTIONAL_SUPPORT;
#else
#if (MD_GENERATION >= 6295) && defined(USIP_SHARE_MEMORY_FEATURE_ENABLE)
	...
	md_feature->feature_set[MEMORY_REMAP_INFO].support_mask =
		CCCI_FEATURE_MUST_SUPPORT;
#endif
#endif
}
		...
		struct ccci_runtime_md_mem_ap_addr tbl[], unsigned int num)
{
	...
	if (add_num < num) {
		...
		CCCI_REPEAT_LOG(md_id, TAG, "%s add bank4 c fail(%d)\n",
			__func__, add_num);
	rt_ft->feature_id = MEMORY_REMAP_INFO;
...
unsigned int ccci_md_get_dhl_dump_config(unsigned char md_id)
{
	...
	if (node) {
		...
		CCCI_NORMAL_LOG(md_id, TAG, "aee_enable=%d\n", ret);
	} else {
	...
	for (i = BOOT_INFO; i < FEATURE_COUNT; i++) {
		...
		CCCI_FEATURE_MUST_SUPPORT) {
			...
			case CCCI_DHL_MISC_INFO:
				...
				/* use the random_seed as temp_u32 value */
				random_seed = ccci_md_get_dhl_dump_config(md_id);
				if (random_seed == 1) {
				...
				append_runtime_feature(&rt_data, &rt_feature,
					&aee_config);
				CCCI_NORMAL_LOG(md_id, TAG, "sync DHL config done\n");
				break;
#endif
#endif
			default:
