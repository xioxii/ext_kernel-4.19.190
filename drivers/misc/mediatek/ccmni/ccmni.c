...
struct proc_dir_entry *ccmni_proc_file;
...
static bool ccmni_is_arp_proxy_addr(int md_id, struct net_device *master, __be32 dest_ip)
{
	...
	for_primary_ifa(in_dev) {
		...
		if (d_addr != (s_addr + 1) && d_addr != (s_addr - 1))
			break;
		...
		struct sk_buff *skb)
{
	...
	if (!master) {
		CCMNI_ERR_MSG(md_id, "No master device\n");
		goto drop_abnormal_arp;
	}
	...
	if (!src_ip || dest_ip == src_ip) {
		CCMNI_INF_MSG(md_id, "ignore arp probe and announcement\n");
	...
	if (!ccmni_is_arp_proxy_addr(md_id, master, dest_ip))
		goto drop_ignored_arp;
	CCMNI_INF_MSG(md_id, "Handle ARP Target:%pI4\n", &dest_ip);
...
static int ccmni_handle_neigh_solicit(struct net_device *dev, struct sk_buff *skb)
{
	...
	pr_err("%s mac:%pM Target:%pI6\n", __func__, eth_hdr(skb)->h_source,
			&ns->target);
	...
	void *_eth_hdr, unsigned char *mac_addr, unsigned int packet_type)
{
	...
	if (IS_CCMNI_LAN(dev)) {
		...
		if (br_mode) {
			struct ccmni_instance *ccmni = (struct ccmni_instance *)
				netdev_priv(dev);
			if (is_zero_ether_addr(ccmni->idu_mac_addr))
				memcpy(eth_hdr->h_dest, mac_addr, sizeof(eth_hdr->h_dest));
			else
				memcpy(eth_hdr->h_dest, ccmni->idu_mac_addr,
						sizeof(eth_hdr->h_dest));
		} else {
			memcpy(eth_hdr->h_dest, mac_addr, sizeof(eth_hdr->h_dest));
		}
...
static inline int ccmni_is_ack_skb(int md_id, struct sk_buff *skb)
{
	u32 packet_type;
	struct tcphdr *tcph;
	int ret = 0;
...
static inline int ccmni_arp_reply(int md_id, struct net_device *dev,
	struct ethhdr *eth, struct sk_buff *skb)
{
	...
	if (htons(request->ar_hrd) != ARPHRD_ETHER ||
		...
		CCMNI_INF_MSG(md_id, "arp_reply: %s not_arp_req, ar_hrd=0x%x, "
				"ar_pro=0x%x, ar_hln=%d, ar_pln=%d=, ar_op=0x%x\n", dev->name,
				htons(request->ar_hrd), htons(request->ar_pro), request->ar_hln,
	...
	if (!new_skb) {
		CCMNI_INF_MSG(md_id, "arp_reply: %s can't alloc new skb\n", dev->name);
		goto alloc_skb_fail;
	}
...
#ifdef ENABLE_WQ_GRO
static int ccmni_is_skb_gro(struct sk_buff *skb)
{
	u32 packet_type;
		...
		struct net_device *sb_dev, select_queue_fallback_t fallback)
{
	...
	if (ctlb->ccci_ops->md_ability & MODEM_CAP_DATA_ACK_DVD) {
		...
		if (ccmni->ack_prio_en && ccmni_is_ack_skb(ccmni->md_id, skb))
			return CCMNI_TXQ_FAST;
		else
...
static int ccmni_open(struct net_device *dev)
{
	...
	if (unlikely(ccmni_ctl == NULL)) {
		CCMNI_PR_DBG(ccmni->md_id, "%s_Open: MD%d ctlb is NULL\n", dev->name,
				ccmni->md_id);
		return -1;
	}
	...
	CCMNI_INF_MSG(ccmni->md_id, "%s_Open:cnt=(%d,%d), md_ab=0x%X, gro=(%llx, "
...
static int ccmni_close(struct net_device *dev)
{
	...
	CCMNI_INF_MSG(ccmni->md_id, "%s_Close:cnt=(%d, %d)\n", dev->name,
			atomic_read(&ccmni->usage), atomic_read(&ccmni_tmp->usage));

	return 0;
}

static int ccmni_tx_profile = 0;
module_param(ccmni_tx_profile, uint, 0644);
MODULE_PARM_DESC (ccmni_tx_profile, "enable ccmni_tx_profile debugging");
struct timespec starttime_ccmni_tx;
static unsigned long long sen_data_len = 0;
...
static int ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	int mac_len = skb_network_offset(skb);
	struct md_tag_packet *tag = NULL;
	unsigned int count = 0;
	struct iphdr *iph;
	struct timespec curr_time, diff;
	...
	if (br_mode) {
		 ...
		 */
		if (IS_CCMNI_LAN(dev)) {
			if (mac_len > 0 && skb->protocol == htons(ETH_P_ARP)) {
				if (ccmni_arp_reply(ccmni->md_id, dev, (struct ethhdr *)
	...
	if (unlikely(skb->len > dev->mtu)) {
		CCMNI_PR_DBG(ccmni->md_id, "CCMNI%d write fail: len(0x%x)>MTU(0x%x, "
	...
	if (unlikely(skb_headroom(skb) < sizeof(struct ccci_header))) {
		CCMNI_PR_DBG(ccmni->md_id, "CCMNI%d write fail: header room(%d) < "
				"ccci_header(%d)\n", ccmni->index, skb_headroom(skb),
				dev->hard_header_len); dev_kfree_skb(skb);
		dev->stats.tx_dropped++;
		return NETDEV_TX_OK;
	}
	...
	if (ctlb->ccci_ops->md_ability & MODEM_CAP_DATA_ACK_DVD) {
		...
		if (skb->mark == APP_VIP_MARK)
			...
			is_ack = ccmni_is_ack_skb(ccmni->md_id, skb);
	}
	...
	if (ret == CCMNI_ERR_MD_NO_READY || ret == CCMNI_ERR_TX_INVAL) {
		...
		CCMNI_DBG_MSG(ccmni->md_id, "[TX]CCMNI%d send tx_pkt=%ld(ack=%d) fail="
	...
	if (ccmni_tx_profile) {
		...
		/*CCMNI_ERR_MSG(ccmni->md_id, "ccmni%d ipid=0x%04x dport=%d skb->len=%d\n",
				ccmni->index,
				ntohs(ip_hdr(skb)->id),
				ntohs(tcp_hdr(skb)->dest),
				skb_len);*/
		if (unlikely(starttime_ccmni_tx.tv_sec == 0)) {
			getnstimeofday(&starttime_ccmni_tx);
		} else {
			getnstimeofday(&curr_time);
			diff = timespec_sub(curr_time, starttime_ccmni_tx);
			if ((diff.tv_sec == 1) || ((diff.tv_sec == 0) && (diff.tv_nsec > 980000000))) {
				CCMNI_ERR_MSG(ccmni->md_id, "[TX] ccmni%d sen_data_len=%llu "
						"duration=%llu(s)/%ld(ns) "
						"speed=%llu mbps/%llu kbps/%llu bps\n",
						ccmni->index,
						sen_data_len,
			...
			if (diff.tv_sec > 1) {
				CCMNI_ERR_MSG(ccmni->md_id, "[TX] ccmni%d sen_data_len=%llu "
						"duration=%llu(s) sen_bits=%llu mb/%llu kb/%llu b\n",
						ccmni->index,
						sen_data_len,
						(long long)diff.tv_sec,
						(sen_data_len >> 20)*8,
	...
	if (ccmni->tx_busy_cnt[is_ack] > 10) {
		CCMNI_DBG_MSG(ccmni->md_id, "[TX]CCMNI%d TX busy: tx_pkt=%ld(ack=%d) "
...
tx_busy:
	if (unlikely(!(ctlb->ccci_ops->md_ability & MODEM_CAP_TXBUSY_STOP))) {
		if ((ccmni->tx_busy_cnt[is_ack]++) % 100 == 0)
			CCMNI_DBG_MSG(ccmni->md_id, "[TX]CCMNI%d TX busy: tx_pkt=%ld "
					"(ack=%d) retry_times=%ld\n", ccmni->index,
					dev->stats.tx_packets, is_ack, ccmni->tx_busy_cnt[is_ack]);
	} else {
...
static void ccmni_tx_timeout(struct net_device *dev)
{
	...
	CCMNI_DBG_MSG(ccmni->md_id, "ccmni%d_tx_timeout: usage_cnt=%d, "
...
static ssize_t ccmni_proc_write(struct file *file,
...
static int ccmni_proc_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", br_mode);
	return 0;
}

static int ccmni_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, ccmni_proc_read, NULL);
}

static const struct file_operations ccmni_fops = {
	.owner      = THIS_MODULE,
	.open		= ccmni_proc_open,
	.write      = ccmni_proc_write,
	.read		= seq_read,
};
	...
	for (i = 0, j = 0; i < MAX_ADDR_LEN; i++, ptr++) {
		...
		if (*ptr == ':' || *ptr == '\0') {
			*ptr = '\0';
			...
			if (ret) {
				pr_err("%s kstrtou8 ret=%d conversion failure for found=[%s] "
...
static const struct file_operations ccmni_idu_fops = {
	.owner      = THIS_MODULE,
	.open		= ccmni_proc_idu_mac_open,
	.write      = ccmni_proc_idu_mac_write,
	.read		= seq_read,
};
	...
	struct net_device *dev)
{
	...
	if (ret) {
		CCMNI_PR_DBG(md_id, "get ccmni%d channel fail\n", ccmni->index);
		return ret;
	}
	...
	snprintf(proc_name, 24, "idu_mac_ccmni%d", ccmni->index);
	if (!ccmni->proc_idu_mac)
		ccmni->proc_idu_mac = proc_create_data(proc_name, 0644,
...
static int ccmni_init(int md_id, struct ccmni_ccci_ops *ccci_info)
{
	...
	if (!ccmni_proc_dir)
		return -ENOMEM;
	if (!ccmni_proc_file)
		ccmni_proc_file = proc_create("bridge_mode_control", 0644,
				ccmni_proc_dir, &ccmni_fops);
	if (!ccmni_proc_file)
		return -ENOMEM;
	...
	if ((ctlb->ccci_ops->md_ability & MODEM_CAP_CCMNI_IRAT) != 0) {
		if (ctlb->ccci_ops->irat_md_id < 0 ||
			...
			CCMNI_PR_DBG(md_id, "md%d IRAT fail: invalid irat md(%d)\n", md_id,
					ctlb->ccci_ops->irat_md_id);
			ret = -EINVAL;
			goto alloc_mem_fail;
		}
		...
		if (!ctlb_irat_src) {
			CCMNI_PR_DBG(md_id, "md%d IRAT fail: irat md%d ctlb is NULL\n",
		...
		if (unlikely(ctlb->ccci_ops->ccmni_num >
			...
			CCMNI_PR_DBG(md_id, "IRAT fail: ccmni number not match(%d, %d)\n",
			ctlb_irat_src->ccci_ops->ccmni_num,
		...
		for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
			...
			if (unlikely(ccmni_irat_src == NULL)) {
				CCMNI_PR_DBG(md_id, "alloc ccmni_irat instance fail\n");
				kfree(ccmni);
				ret = -ENOMEM;
				goto alloc_mem_fail;
			}
	...
	if (ctlb->ccci_ops->md_ability & MODEM_CAP_DIRECT_TETHERING) {
		...
		ret = register_netdev(dev);
		if (ret) {
			CCMNI_PR_DBG(md_id, "CCMNI%d register netdev fail: %d\n", i, ret);
			goto alloc_netdev_fail;
		}
	}
...
static void ccmni_exit(int md_id)
{
	...
	if (ctlb) {
		...
		for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
			ccmni = ctlb->ccmni_inst[i];
			if (ccmni) {
				CCMNI_DBG_MSG(md_id, "%s: unregister ccmni%d dev\n", __func__,
						i);
				unregister_netdev(ccmni->dev);
				/* free_netdev(ccmni->dev); */
				ctlb->ccmni_inst[i] = NULL;
		...
		void *priv_data)
{
	...
	if (unlikely(ctlb == NULL || ctlb->ccci_ops == NULL)) {
		CCMNI_PR_DBG(md_id, "invalid CCMNI%d ctrl/ops struct\n", ccmni_idx);
		dev_kfree_skb(skb);
		return -1;
	}
	...
	if (br_mode && ccmni_is_multicast_addr(skb->data, pkt_type)) {
		ccmni_make_multicast_frame(skb->data, (struct ethhdr *) (skb->data -
				ETH_HLEN), pkt_type);
		skb->pkt_type = PACKET_MULTICAST;
		dev->stats.multicast++;
	} else if (br_mode && pkt_type == 0x60 &&
                        ccmni_is_odu_v6_addr(dev, &((struct ipv6hdr *)(skb->data))->daddr)) {
	...
	if (ccmni_rx_profile) {
		...
		//CCMNI_ERR_MSG(md_id, "ccmni skb->len=%d \n", skb->len);
		if (unlikely(starttime.tv_sec == 0)) {
			getnstimeofday(&starttime);
		} else {
			getnstimeofday(&curr_time);
			diff = timespec_sub(curr_time, starttime);
			if ((diff.tv_sec == 1) || ((diff.tv_sec == 0) && (diff.tv_nsec > 980000000))) {
				CCMNI_ERR_MSG(md_id, "[RX] ccmni%d rec_data_len=%llu duration=%llu(s)/"
						"%ld(ns) speed=%llu mbps/%llu kbps/%llu bps\n",
						ccmni_idx,
						rec_data_len,
						(long long)diff.tv_sec,
						diff.tv_nsec,
