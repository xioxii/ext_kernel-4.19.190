...
int get_ring_usage(int mode, int i)
{
	...
	struct EDMA_rxdesc *rxring;
	struct EDMA_txdesc *txring;
#else
...
#ifdef CONFIG_RAETH_EDMA
		rxring = (struct EDMA_rxdesc *)RX_BASE_PTR0;
#else
...
#ifdef CONFIG_RAETH_EDMA
		if (txring[tx_ctx_idx].txd_info2.DDONE == 1)
#else
...
int tx_ring_read(struct seq_file *seq, void *v)
{
	...
	for (ring_index = 0; ring_index < MAX_TX_RX_RING_NUM; ring_index++) {
		seq_printf(seq, "Show content of Tx ring with index [%d]:\n", ring_index);
	for (i = 0; i < num_tx_desc; i++)
#ifdef CONFIG_RAETH_EDMA
			tx_ring[i] = ei_local->tx_ring[ring_index][i];//todo
#else
		tx_ring[i] = ei_local->tx_ring0[i];
#endif
#ifdef CONFIG_RAETH_EDMA
	    for (i = 0; i < num_tx_desc; i++) {
			seq_printf(seq, "%4d: %08x %08x %08x %08x %08x %08x %08x %08x\n", i,
				   *(int *)&tx_ring[i].txd_info1,
				   *(int *)&tx_ring[i].txd_info2,
				   *(int *)&tx_ring[i].txd_info3,
				   *(int *)&tx_ring[i].txd_info4,
				   *(int *)&tx_ring[i].txd_info5,
				   *(int *)&tx_ring[i].txd_info6,
				   *(int *)&tx_ring[i].txd_info7,
...
int rx_ring_read(struct seq_file *seq, void *v)
{
	...
	for (ring_index = 0; ring_index < MAX_TX_RX_RING_NUM; ring_index++) {
		seq_printf(seq, "Show content of Rx ring with index [%d]:\n", ring_index);
	for (i = 0; i < num_rx_desc; i++) {
			memcpy(&rx_ring[i], &ei_local->rx_ring[ring_index][i],
			       DMA_rxdesc_size);
	}
		...
		for (i = 0; i < num_rx_desc; i++) {
			seq_printf(seq, "%4d: %08x %08x %08x %08x %08x %08x %08x %08x\n", i,
				   ...
				   *(int *)&rx_ring[i].rxd_info5,
				   *(int *)&rx_ring[i].rxd_info6,
				   *(int *)&rx_ring[i].rxd_info7,
...
static struct proc_dir_entry *proc_port_status;
...
int port_status_read(struct seq_file *seq, void *v)
{
	unsigned int ESW_POA;

	ESW_POA = sys_reg_read(ETHDMASYS_ETH_SW_BASE + 0x80);
	seq_puts(seq, "+-----------------------------------------------+\n");
	seq_puts(seq, "|	 <<Port Ability Offset>>	       |\n");
	seq_puts(seq, "+-----------------------------------------------+\n");

	seq_printf(seq, "Control Register Value:%08x \n", ESW_POA);

	seq_printf(seq, "Port0 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x2000000) >> 25 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
			((ESW_POA & 0x1) >> 0 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x10000) >> 16 ? FLOW_CONTROL_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x200) >> 9 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));

	seq_printf(seq, "Port1 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x4000000) >> 26 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
			((ESW_POA & 0x2) >> 1 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x20000) >> 17 ? FLOW_CONTROL_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x400) >> 10 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));

	seq_printf(seq, "Port2 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x8000000) >> 27 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
			((ESW_POA & 0x4) >> 2 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x40000) >> 18 ? FLOW_CONTROL_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x800) >> 11 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));

	seq_printf(seq, "Port3 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x10000000) >> 28 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
			((ESW_POA & 0x8) >> 3 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x80000) >> 19 ? FLOW_CONTROL_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x1000) >> 12 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));

	seq_printf(seq, "Port4 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x20000000) >> 29 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
			((ESW_POA & 0x10) >> 4 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x100000) >> 20 ? FLOW_CONTROL_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x2000) >> 13 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));

	seq_printf(seq, "Port5 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x40000000) >> 30 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
 			((ESW_POA & 0x60) >> 5 == 2 ? SPEED_MODE_1000M :
			  (ESW_POA & 0x60) >> 5 == 1 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x600000) >> 21 == 3 ? FLOW_CONTROL_RXTX_ON :
			  (ESW_POA & 0x600000) >> 21 == 2 ? FLOW_CONTROL_TX_ON :
			  (ESW_POA & 0x600000) >> 21 == 1 ? FLOW_CONTROL_RX_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x4000) >> 14 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));

	seq_printf(seq, "Port6 status=%s, speed=%s, flow control=%s, duplex mode=%s \n",
			((ESW_POA & 0x80000000) >> 31 ? LINK_STATUS_UP : LINK_STATUS_DOWN),
			((ESW_POA & 0x180) >> 7 == 2 ? SPEED_MODE_1000M :
			  (ESW_POA & 0x180) >> 7 == 1 ? SPEED_MODE_100M : SPEED_MODE_10M),
			((ESW_POA & 0x1800000) >> 23 == 3 ? FLOW_CONTROL_RXTX_ON :
			  (ESW_POA & 0x1800000) >> 23 == 2 ? FLOW_CONTROL_TX_ON :
			  (ESW_POA & 0x1800000) >> 23 == 1 ? FLOW_CONTROL_RX_ON : FLOW_CONTROL_OFF),
			((ESW_POA & 0x8000) >> 15 ? DUPLEX_MODE_ON : DUPLEX_MODE_OFF));
	seq_puts(seq, "+-----------------------------------------------+\n");
...
static int port_status_open(struct inode *inode, struct file *file)
{
...
int debug_proc_init(void)
{
	...
	if (ei_local->chip_name == MT7622_FE || ei_local->chip_name == LEOPARD_FE) {
		proc_port_status =
			proc_create(PROCREG_PORT_STATUS, 0, proc_reg_dir, &port_status_fops);
		if (!proc_port_status)
			pr_debug("!! FAIL to create %s PROC !!\n", PROCREG_PORT_STATUS);
	}
...
void debug_proc_exit(void)
{
	...
	if (ei_local->chip_name == MT7622_FE || ei_local->chip_name == LEOPARD_FE) {
		if (proc_port_status)
			remove_proc_entry(PROCREG_PORT_STATUS, proc_reg_dir);
	}
