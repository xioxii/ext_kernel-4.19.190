			 ...
			 struct sk_buff *skb, int gmac_no, int ring_no)
{
	...
	//harry test
	mapped_addr = dma_map_single(&ei_local->qdma_pdev->dev, skb->data, skb->len, DMA_TO_DEVICE);
#if(0)
	/* 2. prepare data */
	...
	//cpu_ptr->txd_info1.SDP = virt_to_phys(skb->data);
	cpu_ptr->txd_info1.SDP = mapped_addr;
	...
	if (cpu_ptr->txd_info6.INSV_1 == 1)
		pr_err("INSV_1 init value is 1\n");
			     ...
			     int gmac_no, int ring_no)
{
	...
	//harry test
	mapped_addr = dma_map_single(&ei_local->qdma_pdev->dev, skb->data, len, DMA_TO_DEVICE);
	 ...
	 */
	if (len > 0) {
		for (frag_txd_num = cal_frag_txd_num(len); frag_txd_num > 0;
		     frag_txd_num--) {
			if (len < MAX_QTXD_LEN)
				size = len;
			else
	...
	for (i = 0; i < nr_frags; i++) {
		...
		len = frag->size;
		for (frag_txd_num = cal_frag_txd_num(len);
	...
	if (skb_shinfo(skb)->gso_segs > 1) {
		...
		if ((iph->version == 4) && (iph->protocol == IPPROTO_TCP)) {
			...
			mapped_addr_th = dma_map_single(&ei_local->qdma_pdev->dev, skb->data,
...
int fe_qdma_rx_dma_init(struct net_device *dev)
{
	...
	for (i = 0; i < NUM_QRX_DESC; i++) {
		...
		memset(&ei_local->qrx_ring[i], 0, DMA_rxdesc_size);
