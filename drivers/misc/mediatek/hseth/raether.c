...
static const char * const mtk_clks_source_name[] = {
	"net_sel", "med_sel", "net_500_sel", "med_mcu_sel",
...
// Pohan SGMII
static int mtk_phy_connect(struct net_device *dev, int id)
{
	...
	pr_info("mtk_phy_connect +,  id=%d\n", id);

	mac = ei_local->mac[id];
	np = of_parse_phandle(mac->of_node, "phy-handle", 0);

	if (ei_local->mac[id] == NULL) {
		pr_info("ei_local->mac[id] == NULL,  id=%d\n", id);
	}

	if (!np && of_phy_is_fixed_link(mac->of_node)) {
		if (!of_phy_register_fixed_link(mac->of_node)) {
			pr_info("of_phy_register_fixed_link,  id=%d\n", id);
	...
	if (of_phy_is_fixed_link(mac->of_node)) {
		pr_info("of_phy_is_fixed_link,  id=%d\n", id);
	...
	// check pseudo net_device if ok
	err = mtk_setup_hw_path(ei_local, mac->id, of_get_phy_mode(np));
...
err_phy:
	...
	of_node_put(np);
	pr_err("%s: invalid phy\n", __func__);
	return -EINVAL;
}


// Pohan SGMII
static int mtk_add_mac(struct END_DEVICE *ei_local, struct device_node *np)
{
	//struct mtk_mac *mac;
	...
	if (!_id) {
		pr_err("missing mac id\n");
		return -EINVAL;
	}
	id = be32_to_cpup(_id);

	pr_info("mtk_add_mac %d \n", id);
	...
	ei_local->mac[id] = devm_kzalloc(ei_local->dev, sizeof(struct mtk_mac), GFP_KERNEL);

	//mac = ei_local->mac[id];
	ei_local->mac[id]->id = id;
	ei_local->mac[id]->of_node = np;
...
static inline void fe_rx_desc_init(struct EDMA_rxdesc *rx_ring,
#else
...
static int rt2880_edma1_eth_recv(struct net_device *dev,
			   struct napi_struct *napi, int budget)
{
	//todo: turn psudo dev to dev
	struct END_DEVICE *ei_local = netdev_priv(dev);
	struct PSEUDO_ADAPTER *p_ad = netdev_priv(ei_local->pseudo_dev);
	struct sk_buff *rx_skb;
	unsigned int length = 0;
	int rx_processed = 0;
	struct EDMA_rxdesc *rx_ring, *rx_ring_next;
	unsigned int rx_dma_owner_idx, rx_next_idx;
	void *rx_data, *rx_data_next, *new_data;
	unsigned int skb_size;
	// Share variable for PDMA and EDMA.
	unsigned int DMA_L4VLD, DMA_TAG, DMA_VID, DMA_SP;

#ifdef CONFIG_RAETH_RW_PDMAPTR_FROM_VAR
	rx_dma_owner_idx = (ei_local->rx_calc_idx[1] + 1) % num_rx_desc; //todo
#else
	rx_dma_owner_idx = (sys_reg_read(RAETH_RX_CALC_IDX0_1) + 1) % num_rx_desc; //todo
#endif
	rx_ring = &ei_local->rx_ring[1][rx_dma_owner_idx];  //todo
	rx_data = ei_local->netrx_skb_data[1][rx_dma_owner_idx];//todo
	...
	for (;;) {
		...
		rx_ring_next = &ei_local->rx_ring[1][rx_next_idx];//todo
		rx_data_next = ei_local->netrx_skb_data[1][rx_next_idx];//todo
		...
		dma_addr = dma_map_single(dev->dev.parent,//todo: should revised to corresponding dev?
		...
		/* Read rxd_info related to PDMA and EDMA */
		DMA_L4VLD = rx_ring->rxd_info3.L4VLD;
		DMA_TAG = rx_ring->rxd_info3.TAG;
		DMA_VID = rx_ring->rxd_info4.VID;
		DMA_SP = rx_ring->rxd_info5.SP;

		/* skb processing */
		skb_put(rx_skb, length);

		/* rx packet from GE2 or EDMA1 */
		if (DMA_SP == 2 || DMA_SP == 12) {
			if (likely(ei_local->pseudo_dev)) {
				...
				pr_info("pseudo_dev is still not initialize ");
				pr_info("but receive packet from GMAC2\n");
			}
		} else {
			rx_skb->dev = dev;
        ...
        if (ppe_hook_rx_eth) {
			if (IS_SPACE_AVAILABLE_HEAD(rx_skb)) {
				*(uint32_t *)(FOE_INFO_START_ADDR_HEAD(rx_skb)) =
					*(uint32_t *)&rx_ring->rxd_info5;
				FOE_ALG(rx_skb) = 0;
				FOE_MAGIC_TAG(rx_skb) = FOE_MAGIC_PCI;
				FOE_TAG_PROTECT(rx_skb) = TAG_PROTECT;
			}
			if (IS_SPACE_AVAILABLE_TAIL(rx_skb)) {
				*(uint32_t *)(FOE_INFO_START_ADDR_TAIL(rx_skb) + 2) =
					*(uint32_t *)&rx_ring->rxd_info5;
				FOE_ALG_TAIL(rx_skb) = 0;
				FOE_MAGIC_TAG_TAIL(rx_skb) = FOE_MAGIC_PCI;
		...
		if (ppe_hook_rx_ext) {
			if (IS_SPACE_AVAILABLE_HEAD(rx_skb)) {
				*(uint32_t *)(FOE_INFO_START_ADDR_HEAD(rx_skb)) =
				...
				FOE_MAGIC_TAG_HEAD(rx_skb) = FOE_MAGIC_EDMARX;
				FOE_TAG_PROTECT_HEAD(rx_skb) = TAG_PROTECT;

				//pr_info("[HS-ethernet/RAETH/RX] modify FOE_ENTRY_NUM/FOE_SP for EDMA\n");
				FOE_ENTRY_NUM(rx_skb) =  rx_ring->rxd_info5.FOE_ENTRY;
				FOE_SP(rx_skb) =  rx_ring->rxd_info5.SP;
				FOE_AI(rx_skb) =  rx_ring->rxd_info5.CRSN;
			...
			if (IS_SPACE_AVAILABLE_TAIL(rx_skb)) {
				*(uint32_t *)(FOE_INFO_START_ADDR_TAIL(rx_skb) + 2) =
					*(uint32_t *)&rx_ring->rxd_info4;
				FOE_ALG_TAIL(rx_skb) = 0;
				FOE_MAGIC_TAG_TAIL(rx_skb) = FOE_MAGIC_EDMARX;
				FOE_TAG_PROTECT_TAIL(rx_skb) = TAG_PROTECT;
 ...
 */
#if defined(CONFIG_RA_HW_NAT)  || defined(CONFIG_RA_HW_NAT_MODULE)
#ifdef CONFIG_RAETH_EDMA
				if ((!ppe_hook_rx_ext) || (ppe_hook_rx_ext && ppe_hook_rx_ext(rx_skb))) {
#else
				if ((!ppe_hook_rx_eth) || (ppe_hook_rx_eth && ppe_hook_rx_eth(rx_skb))) {
			...
			if (!(ei_local->features & FE_SW_LRO)) {
				if (ei_local->features & FE_INT_NAPI)
					...
					//pr_info("[HS-ethernet/RAETH/RX] netif_receive_skb FOE_MAGIC_TAG=%x  FOE_MAGIC_TAG_TAIL=%x \n", FOE_MAGIC_TAG(rx_skb), FOE_MAGIC_TAG_TAIL(rx_skb));
		...
		ei_local->netrx_skb_data[1][rx_dma_owner_idx] = new_data;
		...
		sys_reg_write(RAETH_RX_CALC_IDX0_1, rx_dma_owner_idx);
#ifdef CONFIG_RAETH_RW_PDMAPTR_FROM_VAR
		ei_local->rx_calc_idx[1] = rx_dma_owner_idx;
#endif
...
#else
		rx_dma_owner_idx =
		    (sys_reg_read(RAETH_RX_CALC_IDX0_1) + 1) % num_rx_desc; //todo
#endif
...
skb_err:
	...
	sys_reg_write(RAETH_RX_CALC_IDX0_1, rx_dma_owner_idx);//todo
#ifdef CONFIG_RAETH_RW_PDMAPTR_FROM_VAR
	ei_local->rx_calc_idx[1] = rx_dma_owner_idx;
#endif
...
static int raeth_poll_rx_edma1(struct napi_struct *napi, int budget)
{
	struct END_DEVICE *ei_local =
	    container_of(napi, struct END_DEVICE, napi_rx_edma1);
	struct net_device *netdev = ei_local->netdev;
	unsigned long reg_int_mask_rx;
	unsigned long flags;
	int rx_done = 0;

	rx_done = ei_local->ei_eth_recv_edma1(netdev, napi, budget);
	if (rx_done >= budget)
		return budget;

	napi_complete(napi);

	spin_lock_irqsave(&ei_local->irq_lock_1, flags);

	/* Enable RX interrupt */
	reg_int_mask_rx = sys_reg_read(ei_local->fe_rx_int_enable_1);
	sys_reg_write(ei_local->fe_rx_int_enable_1,
		      (reg_int_mask_rx | RING0_RX_DLY_INT));

	spin_unlock_irqrestore(&ei_local->irq_lock_1, flags);

	return rx_done;
}

static irqreturn_t ei_rx_interrupt_napi_sep_edma1(int irq, void *dev_id) //todo:dev_id may be psuedo, need to get original
{
	...
	reg_int_val = sys_reg_read(ei_local->fe_rx_int_status_1);
	if (likely(reg_int_val & RX_INT_ALL)) {
		if (likely(napi_schedule_prep(&ei_local->napi_rx_edma1))) { //todo:napi_rx_edma1
			spin_lock_irqsave(&ei_local->irq_lock_1, flags);

			/* Clear RX interrupt status */
			sys_reg_write(ei_local->fe_rx_int_status_1, RX_INT_ALL);

			/* Disable RX interrupt */
			reg_int_mask = sys_reg_read(ei_local->fe_rx_int_enable_1);
			sys_reg_write(ei_local->fe_rx_int_enable_1,
				      reg_int_mask & ~(RX_INT_ALL));

			spin_unlock_irqrestore(&ei_local->irq_lock_1, flags);

			__napi_schedule(&ei_local->napi_rx_edma1);//todo:napi_rx_edma1
		}
	} else {
		spin_lock_irqsave(&ei_local->irq_lock_1, flags);
		...
		sys_reg_write(ei_local->fe_rx_int_status_1, reg_int_val);
...
static irqreturn_t ei_tx_interrupt_edma1(int irq, void *dev_id)
{
	struct net_device *netdev = (struct net_device *)dev_id;
	struct net_device *psudo_dev;
	struct END_DEVICE *ei_local;
	unsigned long flags;
	...
	ei_local = netdev_priv(netdev);
	psudo_dev = ei_local->pseudo_dev;

	spin_lock_irqsave(&ei_local->irq_lock_1, flags);

	reg_int_val = sys_reg_read(ei_local->fe_tx_int_status_1);

	if (likely(reg_int_val & TX_INT_ALL)) {
		/* Disable TX interrupt */
		reg_int_mask = sys_reg_read(ei_local->fe_tx_int_enable_1);
		sys_reg_write(ei_local->fe_tx_int_enable_1,
			      reg_int_mask & ~(TX_INT_ALL));
		/* Clear TX interrupt status */
		sys_reg_write(ei_local->fe_tx_int_status_1, TX_INT_ALL);
		//ei_local->ei_xmit_housekeeping(psudo_dev, num_tx_max_process); //todo
		ei_local->ei_xmit_housekeeping_edma1(netdev, num_tx_max_process);
		/* Enable TX interrupt */
		reg_int_mask = sys_reg_read(ei_local->fe_tx_int_enable_1);
		sys_reg_write(ei_local->fe_tx_int_enable_1,
			      reg_int_mask | ei_local->tx_mask_1);
	}
...
static inline void ei_receive_additional(void)
{
	struct net_device *dev = dev_raether;
	struct END_DEVICE *ei_local = netdev_priv(dev);
	struct net_device *psudo_dev = ei_local->pseudo_dev;
	unsigned long reg_int_mask;
	int rx_processed;
	unsigned long flags;

	if (ei_local->tx_ring_full_1 == 0) {
		rx_processed = ei_local->ei_eth_recv(psudo_dev, NULL,//or use another ei_eth_recv_1
						     NUM_RX_MAX_PROCESS);
		if (rx_processed > NUM_RX_MAX_PROCESS) {
			if (likely(ei_local->features & FE_INT_TASKLET))
				tasklet_hi_schedule(&ei_local->rx_tasklet_1);
			else
				schedule_work(&ei_local->rx_wq_1);
		} else {
			spin_lock_irqsave(&ei_local->irq_lock_1, flags);
			/* Enable RX interrupt */
			reg_int_mask = sys_reg_read(ei_local->fe_rx_int_enable_1);
			sys_reg_write(ei_local->fe_rx_int_enable_1,
				      reg_int_mask | ei_local->rx_mask_1);
			spin_unlock_irqrestore(&ei_local->irq_lock_1, flags);
		}
	} else {
		if (likely(ei_local->features & FE_INT_TASKLET))
			tasklet_schedule(&ei_local->rx_tasklet_1);
		else
			schedule_work(&ei_local->rx_wq_1);
	}
}

static void ei_receive_tasklet_additional(unsigned long unused)//todo:this is dev_id
{
	ei_receive_additional(); //todo: para is dev_id
}

static void ei_receive_workq_additional(struct work_struct *work)
{
	ei_receive_additional(); //todo: para is dev_id
}
#endif
			   ...
			   struct napi_struct *napi, int budget)
{
	...
	for (;;) {
		...
		DMA_SP = rx_ring->rxd_info5.SP;
#else
		DMA_L4VLD = rx_ring->rxd_info4.L4VLD;
		DMA_TAG = rx_ring->rxd_info2.TAG;
		DMA_VID = rx_ring->rxd_info3.VID;
		DMA_SP = rx_ring->rxd_info4.SP;
#endif
		...
		if (ppe_hook_rx_ext) {
			if (IS_SPACE_AVAILABLE_HEAD(rx_skb)) {
				*(uint32_t *)(FOE_INFO_START_ADDR_HEAD(rx_skb)) =
				...
				//pr_info("[HS-ethernet/RAETH/RX] modify FOE_ENTRY_NUM/FOE_SP for EDMA1111\n");
				FOE_ENTRY_NUM(rx_skb) =  rx_ring->rxd_info5.FOE_ENTRY;
				FOE_SP(rx_skb) =  rx_ring->rxd_info5.SP;
				FOE_AI(rx_skb) =  rx_ring->rxd_info5.CRSN;
...
static void ei_func_register(struct END_DEVICE *ei_local)
{
	...
	} else if (ei_local->features & FE_EDMA) {
		ei_local->ei_start_xmit = ei_edma_start_xmit;
		ei_local->ei_xmit_housekeeping = ei_edma_xmit_housekeeping;
		RUN_EDMA_CONCURRENT(ei_local->ei_xmit_housekeeping_edma1 = ei_edma1_xmit_housekeeping);
		ei_local->fe_tx_int_status =
		    (void __iomem *)RAETH_FE_INT_STATUS;
		ei_local->fe_tx_int_enable =
		    (void __iomem *)RAETH_FE_INT_ENABLE;
		RUN_EDMA_CONCURRENT(ei_local->fe_tx_int_status_1 =
		    (void __iomem *)RAETH_FE_INT_STATUS_1);
		RUN_EDMA_CONCURRENT(ei_local->fe_tx_int_enable_1 =
		    (void __iomem *)RAETH_FE_INT_ENABLE_1);
	} else {
	...
	if (ei_local->features & FE_QDMA_RX) {
		...
		ei_local->fe_rx_int_enable =
		    (void __iomem *)RAETH_FE_INT_ENABLE;
		RUN_EDMA_CONCURRENT(ei_local->fe_rx_int_status_1 =
		    (void __iomem *)RAETH_FE_INT_STATUS_1);
		RUN_EDMA_CONCURRENT(ei_local->fe_rx_int_enable_1 =
		    (void __iomem *)RAETH_FE_INT_ENABLE_1);
	}
	...
	if (ei_local->features & FE_HW_LRO) {
		...
		ei_local->ei_eth_recv_rss0 = fe_rss0_recv;
		ei_local->ei_eth_recv_rss1 = fe_rss1_recv;
	} else if (ei_local->features & FE_EDMA_CONCURRENT) {
		ei_local->ei_eth_recv = rt2880_eth_recv;
		RUN_EDMA_CONCURRENT(ei_local->ei_eth_recv_edma1 = rt2880_edma1_eth_recv);
	} else {
		ei_local->ei_eth_recv = rt2880_eth_recv;
	}
...
static int __init ei_init(struct net_device *dev)
{
	...
	if (ei_local->features & FE_INT_NAPI) {
		...
		if (ei_local->features & FE_INT_NAPI_TX_RX) {
			...
			if (ei_local->features & FE_RSS_4RING) {
				...
				netif_napi_add(&ei_local->dummy_dev,//todo:all is dummy device, how to separate edma0 & edma1
					       &ei_local->napi_rx,
					       raeth_poll_rx, MTK_NAPI_WEIGHT);
				RUN_EDMA_CONCURRENT(netif_napi_add(&ei_local->dummy_dev,
					       &ei_local->napi_rx_edma1,
					       raeth_poll_rx_edma1, MTK_NAPI_WEIGHT));
			} else {
				netif_napi_add(&ei_local->dummy_dev,
					       &ei_local->napi_rx,
					       raeth_poll_rx, MTK_NAPI_WEIGHT);
	...
	RUN_EDMA_CONCURRENT(spin_lock_init(&ei_local->irq_lock_1));
...
static void fe_dma_rx_cal_idx_init(struct END_DEVICE *ei_local)
{
	if (unlikely(ei_local->features & FE_QDMA_RX)) {
		...
		if (ei_local->features & (FE_HW_LRO | FE_RSS_4RING)) {
			...
			RUN_EDMA_CONCURRENT(ei_local->rx_calc_idx[1] = sys_reg_read(RX_CALC_IDX0_1));
		}
	}
}
#endif
...
static inline int ei_init_ptx_prx(struct net_device *dev)
{
	...
	err = pdma_wait_dma_idle();
	if (err)
		return err;

	err = pdma_rx_dma_init(dev);
	if (err)
		return err;
...
static inline int ei_init_etx_erx(struct net_device *dev)
{
	int err;
	struct END_DEVICE *ei_local = netdev_priv(dev);

	err = fe_edma_wait_dma_idle();
	if (err)
		return err;

	err = fe_edma_rx_dma_init(dev);
	if (err)
		return err;
...
static void ei_deinit_dma(struct net_device *dev)
{
	...
	if (ei_local->features & FE_QDMA_TX) {
		...
		fe_edma_tx_dma_deinit(dev);
		fe_edma_rx_dma_deinit(dev);
	} else {
...
//shooting:todo
	...
	if (ei_local->features & FE_DLY_INT) {
		...
		RUN_EDMA_CONCURRENT(ei_local->tx_mask_1 = RLS_DLY_INT);
		...
		if (ei_local->features & FE_RSS_4RING)
			...
			RUN_EDMA_CONCURRENT(ei_local->rx_mask_1 = RX_DLY_INT);
		}
	} else {
		ei_local->tx_mask = TX_DONE_INT0;
		ei_local->rx_mask = RX_DONE_INT0 | RX_DONE_INT1 | RX_DONE_INT2 | RX_DONE_INT3;
		RUN_EDMA_CONCURRENT(ei_local->tx_mask_1 = TX_DONE_INT0);//todo
		RUN_EDMA_CONCURRENT(ei_local->rx_mask_1 = RX_DONE_INT0 | RX_DONE_INT1 | RX_DONE_INT2 | RX_DONE_INT3);
	}

	/* Enable PDMA interrupts */
	if (ei_local->features & FE_DLY_INT) {
		sys_reg_write(RAETH_DLY_INT_CFG, DELAY_INT_INIT);
		RUN_EDMA_CONCURRENT(sys_reg_write(RAETH_DLY_INT_CFG_1, DELAY_INT_INIT));
		if (ei_local->features & FE_RSS_4RING) {
			...
			RUN_EDMA_CONCURRENT(sys_reg_write(RAETH_FE_INT_ENABLE_1, RAETH_FE_INT_DLY_INIT));
		}
	} else {
		sys_reg_write(RAETH_FE_INT_ENABLE, RAETH_FE_INT_ALL);
		RUN_EDMA_CONCURRENT(sys_reg_write(RAETH_FE_INT_ENABLE_1, RAETH_FE_INT_ALL));
	}
	...
	if (ei_local->features & FE_IRQ_SEPARATE) {
		if (ei_local->features & FE_DLY_INT) {
...
#ifdef CONFIG_RAETH_EDMA
				sys_reg_write(PDMA_INT_GRP1, RX_DLY_INT);
				RUN_EDMA_CONCURRENT(sys_reg_write(EDMA1_INT_GRP1, RX_DLY_INT));
#else
			...
			pr_info("no delay interrupt\n");
			/* PDMA setting */
			sys_reg_write(PDMA_INT_GRP1, TX_DONE_INT0);
			RUN_EDMA_CONCURRENT(sys_reg_write(EDMA1_INT_GRP1, TX_DONE_INT0))
			/* QDMA setting */
			...
			if (ei_local->features & FE_RSS_4RING) {
				...
				RUN_EDMA_CONCURRENT(sys_reg_write(EDMA1_INT_GRP2, RX_DONE_INT0 | RX_DONE_INT1 |
					      RX_DONE_INT2 | RX_DONE_INT3));
			}
		}
	...
	if (ei_local->features & FE_INT_TASKLET) {
		tasklet_init(&ei_local->rx_tasklet, ei_receive_tasklet, 0);//todo: where is dev_id pass? ei_local->rx_tasklet.data
		RUN_EDMA_CONCURRENT(tasklet_init(&ei_local->rx_tasklet_1, ei_receive_tasklet_additional, 0));//todo: where is dev_id pass? ei_local->rx_tasklet.data
	} else if (ei_local->features & FE_INT_WORKQ) {
		INIT_WORK(&ei_local->rx_wq, ei_receive_workq);
		RUN_EDMA_CONCURRENT(INIT_WORK(&ei_local->rx_wq_1, ei_receive_workq_additional));
	} else {
		if (ei_local->features & FE_INT_NAPI_TX_RX) {
			...
			if (ei_local->features & FE_RSS_4RING) {
				...
				napi_enable(&ei_local->napi_rx);
				RUN_EDMA_CONCURRENT(napi_enable(&ei_local->napi_rx_edma1));
			}
		} else {
...
int forward_config(struct net_device *dev)
{
	...
	//sys_reg_write(GDMA1_FWD_CFG, reg_val); //decide by hwnat driver
...
static int ei_clock_enable(struct END_DEVICE *ei_local)
{
	...
	pm_runtime_enable(ei_local->sgmii_phy_dev0);
	pm_runtime_get_sync(ei_local->sgmii_phy_dev0);
	pm_runtime_enable(ei_local->sgmii_phy_dev1);
	pm_runtime_get_sync(ei_local->sgmii_phy_dev1);
	pm_runtime_enable(ei_local->sgmii_dev0);
	pm_runtime_get_sync(ei_local->sgmii_dev0);
	pm_runtime_enable(ei_local->sgmii_dev1);
	pm_runtime_get_sync(ei_local->sgmii_dev1);
	
	clk_prepare_enable(ei_local->clks[MTK_CLK_NETSYS_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_MEDSYS_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_NETSYS_500M_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_NETSYS_MED_MCU_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_NETSYS_WED_MCU_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_NETSYS_2X_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_SGMII_SEL]);
	clk_prepare_enable(ei_local->clks[MTK_CLK_SGMII_SBUS_SEL]);
...
// PoHan SGMII mark out

	if (ei_local->architecture & RAETH_SGMII) {
		clk_disable_unprepare(ei_local->clks[MTK_CLK_SGMII_TX_250M]);
		clk_disable_unprepare(ei_local->clks[MTK_CLK_SGMII_RX_250M]);
...
void raeth_init_pseudo(struct END_DEVICE *p_ad, struct net_device *net_dev)
{
	...
	for (index = 0; index < MAX_PSEUDO_ENTRY; index++) {
		dev = alloc_etherdev_mqs(sizeof(struct PSEUDO_ADAPTER),
					 gmac2_txq_num, 1);
		if (!dev) {
			pr_info("alloc_etherdev for PSEUDO_ADAPTER failed.\n");
			return;
		}
...
int ei_open(struct net_device *dev)
{
	...
	if (!ei_local) {
		pr_info("%s: ei_open passed a non-existent device!\n",
		       dev->name);
		return -ENXIO;
	}

	if (!try_module_get(THIS_MODULE)) {
		pr_info("%s: Cannot reserve module\n", __func__);
		return -1;
	}
...
// set forward port
#ifdef CONFIG_RAETH_EDMA
	// may not need if test with PCIE AQR
	sys_reg_write(RALINK_FRAME_ENGINE_BASE + 0x500, 0x2071bbbb);//todo:tmp default edma
	sys_reg_write(RALINK_FRAME_ENGINE_BASE + 0x1500,0x2071cccc);//todo:tmp default edma1

#else
	sys_reg_write(RALINK_FRAME_ENGINE_BASE + 0x500, 0x20710000);//todo:tmp default edma
	sys_reg_write(RALINK_FRAME_ENGINE_BASE + 0x1500,0x20710000);//todo:tmp default edma1

#endif

/*

	pr_info("!!! MD TX CR 0x15104000: 0x%08x 0x%08x 0x%08x 0x%08x!!\n"
		, sys_reg_read(0x15104000) , sys_reg_read(0x15104004)
		, sys_reg_read(0x15104008), sys_reg_read(0x1510400C));
	pr_info("!!! MD RX CR 0x15104100: 0x%08x 0x%08x 0x%08x 0x%08x!!\n"
		, sys_reg_read(0x15104100) , sys_reg_read(0x15104104)
		, sys_reg_read(0x15104108), sys_reg_read(0x1510410C));
...
#ifdef CONFIG_RAETH_EDMA
	ei_local->skb_free[0] = kmalloc_array(num_tx_desc, sizeof(struct sk_buff *), GFP_KERNEL);
	ei_local->skb_free[1] = kmalloc_array(num_tx_desc, sizeof(struct sk_buff *), GFP_KERNEL);
#else
...
static int rather_probe(struct platform_device *pdev)
{
	...
	const char *mac_addr;
	int ret, err0, err1, err2, err3, i;

	// Pohan SGMII
	int err;
	u32 reg_value;

	void __iomem *gpio_base_virt = ioremap(0x10005370, 0x100);

	DECLARE_EDMA_CONCURRENT(int err4)
	DECLARE_EDMA_CONCURRENT(int err5)

	// gpio
	__raw_writel(0x01111111, gpio_base_virt);

	pr_info("!!!!!!!!!!!!!rather_probe!!!!!!!!!!!!!!!!!\n");

	netdev = alloc_etherdev_mqs(sizeof(struct END_DEVICE), 1, 1);

	//netdev = devm_kzalloc(&pdev->dev, sizeof(struct END_DEVICE), GFP_KERNEL);
	if (!netdev) {
	...
	// PoHan SGMII
	ei_local->ethsys = syscon_node_to_regmap(node);
	ei_local->base = ethdma_frame_engine_base;
	...
	ethdma_mac_base = ioremap(0x1b110000, 0x300);
		...
		ei_local->sgmii_node0 = of_find_compatible_node(NULL, NULL,
						"mediatek,colgin-sgmiisys_0");
						
		if (IS_ERR(ei_local->sgmii_node0)) {
			dev_err(&pdev->dev, "no eth->sgmii_node0 regmap found\n");
			return PTR_ERR(ei_local->sgmii_node0);
		}		
		sgmii_pdev = of_find_device_by_node(ei_local->sgmii_node0);
		ei_local->sgmii_dev0 = &sgmii_pdev->dev;
		ei_local->sgmii_node1 = of_find_compatible_node(NULL, NULL,
						"mediatek,colgin-sgmiisys_1");
		sgmii_pdev = of_find_device_by_node(ei_local->sgmii_node1);
		ei_local->sgmii_dev1 = &sgmii_pdev->dev;
		if (IS_ERR(ei_local->sgmii_node1)) {
			dev_err(&pdev->dev, "no eth->sgmii_node1 regmap found\n");
			return PTR_ERR(ei_local->sgmii_node1);
		}	
		ei_local->sgmii_phy_node0 = of_find_compatible_node(NULL, NULL,
						"mediatek,colgin-sgmiisys_phy_0");
		sgmii_pdev = of_find_device_by_node(ei_local->sgmii_phy_node0);
		ei_local->sgmii_phy_dev0 = &sgmii_pdev->dev;
		if (IS_ERR(ei_local->sgmii_phy_node0)) {
			dev_err(&pdev->dev, "no eth->sgmii_phy_node0 regmap found\n");
			return PTR_ERR(ei_local->sgmii_phy_node0);
		}	
		ei_local->sgmii_phy_node1 = of_find_compatible_node(NULL, NULL,
						"mediatek,colgin-sgmiisys_phy_1");
		sgmii_pdev = of_find_device_by_node(ei_local->sgmii_phy_node1);
		ei_local->sgmii_phy_dev1 = &sgmii_pdev->dev;
		if (IS_ERR(ei_local->sgmii_phy_node1)) {
			dev_err(&pdev->dev, "no eth->sgmii_phy_node1 regmap found\n");
			return PTR_ERR(ei_local->sgmii_phy_node1);
		}
	...
	for (i = 0; i < ARRAY_SIZE(ei_local->clks); i++) {
		pr_info("!!! mtk_clks_source_name[%d] = %s\n", i, mtk_clks_source_name[i]);
		ei_local->clks[i] = devm_clk_get(&pdev->dev, mtk_clks_source_name[i]);
			if (IS_ERR(ei_local->clks[i])) {
	...
	pr_info("Start SGMII config!\n");

	// Pohan SGMII
	//ei_local->soc = of_device_get_match_data(&pdev->dev);
	ei_local->soc = devm_kzalloc(ei_local->dev, sizeof(*ei_local->soc),
								GFP_KERNEL);
	ei_local->soc->caps = 0;
	ei_local->soc->caps |= MT6890_CAPS;
	ei_local->soc->caps |= MTK_HWLRO;
	ei_local->soc->ana_rgc3 = 0x128;



	if (MTK_HAS_CAPS(ei_local->soc->caps, MTK_SGMII)) {
		ei_local->sgmii = devm_kzalloc(ei_local->dev, sizeof(*ei_local->sgmii),
								GFP_KERNEL);

		if (!ei_local->sgmii)
			return -ENOMEM;

		err = mtk_sgmii_init(ei_local->sgmii, pdev->dev.of_node,
							ei_local->soc->ana_rgc3);
	...
	if (ei_local->features & FE_EDMA_CONCURRENT) {
		RUN_EDMA_CONCURRENT(ei_local->irq4 = platform_get_irq(pdev, 4));
		RUN_EDMA_CONCURRENT(ei_local->irq5 = platform_get_irq(pdev, 5));
	}

	pr_info("ei_local->irq0 = %d\n", ei_local->irq0);
	pr_info("ei_local->irq1 = %d\n", ei_local->irq1);
	pr_info("ei_local->irq2 = %d\n", ei_local->irq2);
	pr_info("ei_local->irq3 = %d\n", ei_local->irq3);
	RUN_EDMA_CONCURRENT(pr_info("ei_local->irq4 = %d\n", ei_local->irq4));
	RUN_EDMA_CONCURRENT(pr_info("ei_local->irq5 = %d\n", ei_local->irq5));
	...
	*/
	...
	for_each_child_of_node(pdev->dev.of_node, mac_np) {
		...
		err = mtk_add_mac(ei_local, mac_np);
		if (err)
			pr_err("for_each_child_of_node error!!!!!!!!!!!!!! \n");
	}
...
//harry test for irq
	...
	err0 = devm_request_irq(&pdev->dev, ei_local->irq0, ei_interrupt_napi_rx_only, IRQF_NO_SUSPEND | IRQF_TRIGGER_NONE, "eth2", netdev);
	err1 = devm_request_irq(&pdev->dev, ei_local->irq1, ei_tx_interrupt, IRQF_NO_SUSPEND | IRQF_TRIGGER_NONE, "eth_tx", netdev);
	err2 = devm_request_irq(&pdev->dev, ei_local->irq2, ei_rx_interrupt_napi_sep, IRQF_NO_SUSPEND | IRQF_TRIGGER_NONE, "eth_rx", netdev);
	err3 = devm_request_irq(&pdev->dev, ei_local->irq3, ei_interrupt_napi, IRQF_NO_SUSPEND | IRQF_TRIGGER_NONE, "pcie_test", netdev);
	RUN_EDMA_CONCURRENT(err4 = devm_request_irq(&pdev->dev, ei_local->irq4, ei_tx_interrupt_edma1, IRQF_NO_SUSPEND | IRQF_TRIGGER_NONE, "edma1_tx", netdev));
	RUN_EDMA_CONCURRENT(err5 = devm_request_irq(&pdev->dev, ei_local->irq5, ei_rx_interrupt_napi_sep_edma1, IRQF_NO_SUSPEND | IRQF_TRIGGER_NONE, "edma1_rx", netdev));

	pr_info("err0 =%d, err1 = %d, err2 = %d, err3 = %d\n", err0, err1, err2, err3);
	RUN_EDMA_CONCURRENT(pr_info("err4 =%d, err5 = %d\n", err4, err5));
	//fe_int_enable(netdev);
	...
	// Pohan Colgin MAC
	sys_reg_write(ETHDMASYS_ETH_MAC_BASE + 0x100, 0x2105f33b); // 2105f33b   // Origin: 0x2105e303
	sys_reg_write(ETHDMASYS_ETH_MAC_BASE + 0x200, 0x2105e33b);// 2105e33b  // Origin: 0x2105e303


 	/* SGMII_CONFIG_0 SGMII_CONFIG_1 set as 1  */
	reg_value = sys_reg_read(RALINK_SYSCTL_BASE + 0x14);
	reg_value |= (0x3 << 8);
	// reg_value &= ~(0x3 << 8);
	sys_reg_write(RALINK_SYSCTL_BASE + 0x14, reg_value);


	// Pohan SGMII init for 2 SGMII
	if (!mtk_phy_connect(ei_local->netdev, 0)) {
		pr_info("GE0 mtk_phy_connect good \n");
	} else {
		pr_info("GE0 mtk_phy_connect error \n");
	}

	if (ei_local->features & FE_GE2_SUPPORT) {
		if (ei_local->pseudo_dev) {
			if (!mtk_phy_connect(ei_local->netdev, 1)) {
				pr_info("GE1 mtk_phy_connect good \n");
			} else {
				pr_info("GE1 mtk_phy_connect error \n");
			}
...
*/
	...
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 1, mt_get_ckgen_freq(1)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 38, mt_get_ckgen_freq(38)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 39, mt_get_ckgen_freq(39)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 47, mt_get_ckgen_freq(47)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 48, mt_get_ckgen_freq(48)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 49, mt_get_ckgen_freq(49)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 50, mt_get_ckgen_freq(50)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 51, mt_get_ckgen_freq(51)); 
	pr_info("mt_get_ckgen_freq(%d) = %u\n", 52, mt_get_ckgen_freq(52)); 
