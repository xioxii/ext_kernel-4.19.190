...
int pdma_rx_dma_init(struct net_device *dev)
{
	int i;
...
int pdma_tx_dma_init(struct net_device *dev)
{
	int i;
	struct END_DEVICE *ei_local = netdev_priv(dev);
	...
	if (skb_shinfo(skb)->gso_segs > 1) {
		...
		if ((iph->version == 4) && (iph->protocol == IPPROTO_TCP)) {
			...
			//dma_sync_single_for_device(dev->dev.parent,
						   ...
						   //DMA_TO_DEVICE);
			mapped_addr_th = dma_map_single(dev->dev.parent, skb->data,
...
#else
int pdma_wait_dma_idle(void){return 0;}
int pdma_rx_dma_init(struct net_device *dev){return 0;}
int pdma_tx_dma_init(struct net_device *dev){return 0;}
void fe_pdma_rx_dma_deinit(struct net_device *dev){}
void fe_pdma_tx_dma_deinit(struct net_device *dev){}
void fe_pdma_glo_cfg(void){}
int ei_pdma_start_xmit(struct sk_buff *skb, struct net_device *dev,
		       int gmac_no){return 0;}
