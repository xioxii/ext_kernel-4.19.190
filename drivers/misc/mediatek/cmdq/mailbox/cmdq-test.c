...
static void cmdq_test_mbox_gpr_sleep(struct cmdq_test *test, const bool sleep)
{
	...
	dev = cmdq_register_device(test->dev);

	writel(0x80000000, (void *)CMDQ_TPR_MASK(test->gce.va));
	writel(1 << CMDQ_GPR_DEBUG_TIMER,
		(void *)CMDQ_TPR_TIMEOUT_EN(test->gce.va));

	pkt = cmdq_pkt_create(test->clt, PAGE_SIZE);
	if (!sleep) {
		cmdq_pkt_write_ex(pkt, NULL, CMDQ_TPR_TIMEOUT_EN(test->gce.pa),
	...
	*(out_va + 2) = 0;
	...
	if (!sleep) {
		...
		cmdq_pkt_write_indriect(pkt, NULL, out_pa, CMDQ_TPR_ID, ~0);
		cmdq_pkt_sleep(pkt, dev, 100, CMDQ_GPR_DEBUG_TIMER);
		cmdq_pkt_write_indriect(pkt, NULL, out_pa + 4, CMDQ_TPR_ID, ~0);
	...
	const unsigned long pa, const u32 pttn, const u32 mask, const bool aee)
{
	...
	*(out_va + 1) = 0;
	...
	cmdq_pkt_poll_timeout(pkt, NULL, pttn & mask,
		pa, mask, aee ? U16_MAX : 100, CMDQ_GPR_DEBUG_TIMER);
...
static void cmdq_test_mbox_large_cmd(struct cmdq_test *test)
{
	...
	for (i = 0; i < 64 * 1024 / 8; i++) // 64k instructions
		cmdq_pkt_write_ex(pkt, NULL, pa, i, ~0);
	if (!perf_en)
...
static void cmdq_test_mbox_sync_token_loop_iter(struct timer_list *t)
{
	...
	if (!gtest->tick)
		...
		mod_timer(&cmdq_tltm.test_timer,
			jiffies + msecs_to_jiffies(300));
		gtest->iter += 1;
	}

	/* TODO: Fix timer_setup for 4.19
	if (!gtest->tick)
...
static void cmdq_test_mbox_loop(struct cmdq_test *test)
{
	...
	pkt = cmdq_pkt_create(test->loop, PAGE_SIZE);
	cmdq_pkt_wfe(pkt, test->token_user0);
	...
	*/
	...
	cmdq_tltm.event = test->token_user0;
	timer_setup(&cmdq_tltm.test_timer, cmdq_test_mbox_sync_token_loop_iter, 0);
	...
	clk_disable_unprepare(test->gce.clk);
	test->tick = false;
	del_timer(&cmdq_tltm.test_timer); /* TODO: Fix timer_setup for 4.19 */
}
	...
	struct cmdq_test *test, const bool secure, const bool threaded)
{
	...
	test->tick = true;
	cmdq_ttm.event = test->token_user0;
	timer_setup(&cmdq_ttm.test_timer,
		cmdq_test_mbox_sync_token_flush, 0);
	...
	*/
	...
	test->tick = false;
	del_timer(&cmdq_ttm.test_timer); /* TODO: Fix timer_setup for 4.19 */
	...
	struct cmdq_test *test, const bool secure, const bool need_mask)
{
	...
	cmdq_pkt_write_ex(pkt, NULL, pa, pttn, mask);
	cmdq_pkt_flush(pkt);
...
static void cmdq_test_mbox_stop(struct cmdq_test *test)
{
	...
	for (i = 0; i < ARRAY_SIZE(pkt); i++) {
		pkt[i] = cmdq_pkt_create(test->clt, PAGE_SIZE);
		cmdq_pkt_wfe(pkt[i], test->token_user0);
		cmdq_pkt_flush_async(pkt[i], NULL, NULL);
	}
...
static int cmdq_test_probe(struct platform_device *pdev)
{
	...
	// clt
	test->clt = cmdq_mbox_create(&pdev->dev, 0, CMDQ_TIMEOUT_DEFAULT);
	...
	test->loop = cmdq_mbox_create(&pdev->dev, 1, CMDQ_TIMEOUT_DEFAULT);
...
#ifdef CMDQ_SECURE_SUPPORT
	test->sec = cmdq_mbox_create(&pdev->dev, 2, CMDQ_TIMEOUT_DEFAULT);
