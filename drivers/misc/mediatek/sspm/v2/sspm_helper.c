...
/* clear sspm reg when ipi finished */
int sspm_clr_spm_reg(void *unused)
{
	void __iomem *sspm2spm = sspmreg.cfg + SSPM_CFG_MD2SPM_IPC;
	/* AP side write 0x0 to SSPM2SPM_IPC_CLR to clear reg
	 * SSPM side write 0x1 to SCP2SPM_IPC_SET to set reg
	 * SSPM set        bit[0]
	 */
	writel(0x0, sspm2spm);
...
EXPORT_SYMBOL_GPL(sspm_clr_spm_reg);

/* get sspm reg value */
unsigned int sspm_get_reg(unsigned int offset)
{
	void __iomem *reg = sspmreg.cfg + offset;

	return readl(reg);
}
EXPORT_SYMBOL_GPL(sspm_get_reg);

#ifdef CONFIG_PM
static int sspm_pm_suspend(struct device *dev)
{
	sspm_timesync_suspend();
	return 0;
}

static int sspm_pm_resume(struct device *dev)
{
...
static const struct dev_pm_ops sspm_dev_pm_ops = {
	.suspend = sspm_pm_suspend,
	.resume  = sspm_pm_resume,
};
#endif
...
static int __init sspm_module_init(void)
{
	...
	if (sspm_timesync_init())
		...
		pr_info("[SSPM] Timesync init successfully\n");
