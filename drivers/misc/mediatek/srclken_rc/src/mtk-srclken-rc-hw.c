...
static bool other_module_enable[4];
...
static const char *other_mod_en[MAX_BASE_NUM] = {
	[SRCLKEN_BASE] = "",
	[SCPDVFS_BASE] = "srclken-scp-enable",
	[PWRAP_BASE] = "srclken-pwrap-enable",
	[GPIO_BASE] = "srclken-gpio-enable",
};
...
{
	if (!other_module_enable[GPIO_BASE])
		return;
...
static u32 __srclken_dump_sta(char *buf, u8 id)
{
	...
	switch (id) {
	case XO_SOC:
		len += __subsys_ctl_show(buf + len, SYS_SUSPEND);
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!other_module_enable[SCPDVFS_BASE])
		return 0;
...
#if defined(CONFIG_OF)
static int _srclken_dts_map_internal(struct device_node *node, int idx)
{
	char *buf;
	int ret;
...
static void _srclken_dts_other_module(struct device_node *node, u32 base_idx)
{
	int ret;
	const char *other_en;
	if (base_idx == SRCLKEN_BASE) {
		other_module_enable[SRCLKEN_BASE] = true;
		return;
	}
	ret = of_property_read_string(node, other_mod_en[base_idx], &other_en);
	if (ret || strcmp(other_en, "y"))
		other_module_enable[base_idx] = false;
	else
		other_module_enable[base_idx] = true;
}
...
int srclken_dts_map(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(node, "srclken-mode", &prop);
	if (ret) {
		rc_stage = SRCLKEN_BRINGUP;
		return 0;
	} else if (!strcmp(prop, "bt-only")) {
		rc_stage = SRCLKEN_BT_ONLY;
	} else if (!strcmp(prop, "full-set")) {
	...
	hw = kzalloc(sizeof(struct srclken_hw), GFP_KERNEL);
	if (!hw)
		goto hw_no_mem;
	...
	for (i = 0; i < MAX_BASE_NUM; i++) {
		...
		_srclken_dts_other_module(node, i);
		if (!other_module_enable[i])
			continue;
...
void srclken_stage_init(void)
{
	...
	} else if (rc_stage == SRCLKEN_COANT_ONLY) {
		for (i = 0; i < MAX_SYS_NUM; i++) {
			cfg = srclken_read(RC_M00_SRCLKEN_CFG + i * 4);
	...
	} else if (rc_stage == SRCLKEN_FULL_SET) {
		for (i = 0; i < MAX_SYS_NUM; i++) {
			cfg = srclken_read(RC_M00_SRCLKEN_CFG + i * 4);
