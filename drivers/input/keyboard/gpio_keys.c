...
//For Colgin check gpio status +++
static ssize_t gpio_keys_show_status(struct device *dev,
	struct device_attribute *attr,
	char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	...
	for (i = 0; i < ddata->pdata->nbuttons; i++) {
		struct gpio_button_data *bdata = &ddata->data[i];
		status = gpiod_get_value_cansleep(bdata->gpiod);
		pr_info("[keypad][gpio-keys] code=%d, status=%d\n",
			*bdata->code, status);
...
static DEVICE_ATTR(status, S_IRUGO, gpio_keys_show_status, NULL);
//For Colgin check gpio status ---
...
static struct attribute *gpio_keys_attrs[] = {
	...
	&dev_attr_status.attr, //For Colgin check gpio status
	NULL,
};
...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	if (type == EV_ABS) {
		if (state){
			pr_info("[keypad][gpio-keys] (%s) code=%d\n",
				(state) ? "pressed" : "released", button->code);
			input_event(input, type, button->code, button->value);
		}
	} else {
		pr_info("[keypad][gpio-keys] (%s) code=%d\n",
			(state) ? "pressed" : "released", *bdata->code);
...
/*begin: modify for sleep and wake-up,by laizhenhao,2021.07.30*/
	/* Report current state of buttons that are connected to GPIOs */
	//gpio_keys_report_state(ddata);
/*end: modify for sleep and wake-up,by laizhenhao,2021.07.30*/
	return 0;
}
